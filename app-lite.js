(() => {
  ...
  const API = '/api';
  const ACTIVE_QUOTE_MS = 2500;
  const WATCHLIST_QUOTE_MS = 8000;
  const MIN_QUOTE_MS = 1500;
  const FAST_QUOTE_MS = 2000;
  const SLOW_QUOTE_MS = 4000;
  const TRADE_MOBILE_BREAKPOINT = 768;
  const ACCOUNT_POLL_MS = 25000;
  const FINANCE_POLL_MS = 30000;
  ...
  let __vpVisDebounce = 0;
  document.addEventListener('visibilitychange', () => {
    if (document.hidden) return;
    clearTimeout(__vpVisDebounce);
    __vpVisDebounce = setTimeout(handleVisibilityChange, 400);
  });
  ...
  async function init() {
    ...
    try {
      ...
      applyRoute();
      let __hashDebounce = 0;
      window.addEventListener('hashchange', () => {
        clearTimeout(__hashDebounce);
        __hashDebounce = setTimeout(applyRoute, 180);
      });
    } catch (err) {
  ...
  function updatePositions() {
    ...
    if (!tbody || tbody.children.length !== positions.length) {
      node.innerHTML = positionsTable(positions);
      bindCloseButtons(node);
      return;
    }
    // Incremental update: only mark prices + PnL cells to avoid full reflow
    const rows = tbody.children;
    for (let i = 0; i < positions.length; i++) {
      const p = positions[i];
      const tr = rows[i];
      if (!tr) continue;
      const markCell = tr.querySelector('[data-cell-mark]');
      const pnlCell = tr.querySelector('[data-cell-pnl]');
      if (markCell) markCell.textContent = price(p.mark_price || 0, p.asset_type);
      if (pnlCell) {
    ...
    return `<table class="data-table">
      ...
      <tbody>${items.map((p) => `
        <tr>
          ...
          <td data-cell-mark>${price(p.mark_price || 0, p.asset_type)}</td>
          <td data-cell-pnl class="${Number(p.unrealized_pnl || 0) >= 0 ? 'pos' : 'neg'}">${money(p.unrealized_pnl || 0)}</td>
  ...
  async function api(path, options = {}) {
    const dedupKey = (path + (options.body || '')).replace(/[&?]_=\d+/, '');
    const existing = runtime.inFlight.get(dedupKey);
    ...
    const promise = (async () => {
      try {
        ...
        runtime.inFlight.delete(dedupKey);
      }
    })();
    runtime.inFlight.set(dedupKey, promise);
    return promise;
  }
  ...
  function isAbort(err) {
    return err && (err.name === 'AbortError' || err.name === 'TimeoutError' || /aborted|timeout|signal/i.test(err.message || ''));
  }
