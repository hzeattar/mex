...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
  try {
    ...
    if ($act === 'save_note') {
      ...
      // Credit the user's wallet FIRST (inside the transaction) so the ledger
      // entry and deposit status update are atomic.  If ledger_add throws,
      // the whole transaction rolls back and the deposit stays pending.
