  ...
  return `<div class="space-y-3 order-ticket-panel" data-order-form>
    ...
    <div class="grid grid-cols-2 gap-2">
      ...
      <div class="mobile-order-summary">
        <span><small>Mode</small><strong data-order-mode>${esc(get('mode') === 'real' ? 'Real' : 'Demo')}</strong></span>
        <span><small>Symbol</small><strong data-order-symbol>${esc(get('symbol') || '--')}</strong></span>
        <span><small>Type</small><strong data-order-type-label>${isPerp ? 'Perp/Futures' : 'Spot'}</strong></span>
      </div>
      ...
      <div class="sticky top-0 z-10 flex items-center justify-between gap-3 border-b border-line bg-panel px-4 py-3">
        <div class="flex items-center gap-2 min-w-0">
          ...
          <div class="min-w-0">
            ...
            <strong class="text-sm truncate" data-mobile-order-symbol>${esc(symbol)}</strong>
          </div>
        </div>
...
function startLiveQuotes(container, marketItems, runId = tradeRunId) {
  ...
  }, null, { interval: quoteType === 'crypto' ? 12000 : 18000, initialDelay: 1800, fallbackAfter: 3500, maxSymbols: max });
}
...
function startActiveQuote(container, symbol, type, runId = tradeRunId) {
  ...
  // Non-crypto (forex/stocks/commodities/futures): slower 9s poll, 12s timeout + 1 retry
  const interval   = quoteType === 'crypto' ? 4500 : 9000;
  ...
  const poll = async () => {
    ...
    try {
      const url = `/quotes.php?symbol=${encodeURIComponent(symbol)}&type=${encodeURIComponent(quoteType)}&purpose=focus&_=${Date.now()}`;
      const data = await api(url, { timeout: reqTimeout, retry: reqRetry, signal: activeQuoteController.signal, cacheTtl: 0, cache: 'no-store' });
      if (!isCurrentRun(runId, symbol, type)) return;
...
function updateOrderInfo(container) {
  ...
  const currentSymbol = String(q.symbol || get('symbol') || '--').toUpperCase();
  const currentType = get('type');
  const currentMode = get('mode') === 'real' ? 'Real' : 'Demo';
  ...
  $$('[data-order-form]', container).forEach(form => {
    ...
    const modeEl = $('[data-order-mode]', form);
    if (modeEl) modeEl.textContent = currentMode;
    const symbolEl = $('[data-order-symbol]', form);
    if (symbolEl) symbolEl.textContent = currentSymbol;
    const typeLabelEl = $('[data-order-type-label]', form);
    if (typeLabelEl) typeLabelEl.textContent = marketType === 'perp' ? 'Perp/Futures' : 'Spot';
  });
  const sheetSymbolEl = $('#mobile-order-sheet [data-mobile-order-symbol]', container);
  if (sheetSymbolEl) sheetSymbolEl.textContent = currentSymbol;
  const sheetModeLabel = $('#mobile-order-sheet [data-order-mode]', container);
  if (sheetModeLabel) sheetModeLabel.textContent = currentMode;
  const sheetTypeLabel = $('#mobile-order-sheet [data-order-type-label]', container);
  if (sheetTypeLabel) sheetTypeLabel.textContent = (get('market') || 'spot') === 'perp' ? 'Perp/Futures' : 'Spot';
  const marketSymbolLogo = $('#mobile-order-sheet img[alt]', container);
  if (marketSymbolLogo && currentSymbol !== '--') {
    const logo = marketLogo(currentSymbol, currentType, 'w-8 h-8 rounded-lg shrink-0');
    const tmp = document.createElement('div');
    tmp.innerHTML = logo;
    const nextImg = tmp.querySelector('img');
    const nextFallback = tmp.querySelector('b');
    const currentWrap = marketSymbolLogo.closest('span');
    if (currentWrap && nextImg) {
      const currentImg = currentWrap.querySelector('img');
      const currentFb = currentWrap.querySelector('b');
      if (currentImg) {
        currentImg.src = nextImg.src;
        currentImg.alt = nextImg.alt;
      }
      if (currentFb && nextFallback) currentFb.textContent = nextFallback.textContent;
    }
  }
}
...
async function warmVisibleQuotes(container, items, runId = tradeRunId) {
  ...
  const warmTimeout = type === 'crypto' ? 3000 : 10000; // generous for slow REST providers
...
function updateSymbolListPrices(container, items) {
  ...
  items.forEach(q => {
    ...
    if (priceCell) priceCell.textContent = price(p, type);
    if (changeCell) {
      changeCell.textContent = pct(chg);
      changeCell.className = `text-[9px] ${chg >= 0 ? 'text-buy' : 'text-sell'}`;
    }
    if (statusDot) {
      statusDot.className = `status-dot ${quoteClass(q)}`;
    }
  });
}
...
function renderPositionsMarkup(positions) {
  const cardItems = positions.slice(0, 12);
  const rowItems = positions.slice(0, 12);
  return `
    <div class="trade-position-cards lg:hidden">
      ${cardItems.map(tradePositionCard).join('')}
    </div>
    <div class="hidden lg:block overflow-x-auto"><table class="min-w-[620px] lg:min-w-0 w-full text-[11px]">
      ...
      <tbody>${rowItems.map(tradePositionRow).join('')}</tbody>
    </table></div>`;
}
...
function renderOrdersMarkup(orders) {
  const cardItems = orders.slice(0, 16);
  const rowItems = orders.slice(0, 16);
  return `
    <div class="trade-position-cards lg:hidden">
      ${cardItems.map(tradeOrderCard).join('')}
    </div>
    <div class="hidden lg:block overflow-x-auto"><table class="min-w-[820px] lg:min-w-0 w-full text-[11px]">
      ...
      <tbody>${rowItems.map(tradeOrderRow).join('')}</tbody>
    </table></div>`;
}
...
function tradePositionRow(pos) {
  ...
  return `<tr class="border-t border-line/50 hover:bg-panel/50">
    ...
    <td class="text-muted">${isPerp ? 'Perp' : 'Spot'}</td>
  ...
  return `<article class="trade-position-card">
    <div class="flex items-start justify-between gap-2">
      <div>
        ...
        <small>${esc(isPerp ? 'Perp/Futures' : 'Spot')} - ${esc(pos.created_at || pos.opened_at || '')}</small>
      </div>
      ${sideBadge(side)}
    </div>
    <div class="trade-position-metrics">
      ...
      ${isPerp ? `<span><small>Leverage</small><strong>${esc(String(pos.leverage || 1))}x</strong></span>` : ''}
      ${isPerp && pos.liquidation_price ? `<span><small>Liq. Price</small><strong class="text-sell">${price(pos.liquidation_price, posType)}</strong></span>` : ''}
    </div>
...
function openOrderSheet(container, side) {
  ...
  const currentSymbol = String(get('symbol') || '--').toUpperCase();
  const sheetSymbol = $('[data-mobile-order-symbol]', sheet);
  if (sheetSymbol) sheetSymbol.textContent = currentSymbol;
...
function isExecutableQuote(q) {
  const label = quoteStateLabel(q).toLowerCase();
  return !['reference', 'stale', 'unavailable', 'chart quote'].includes(label);
}
