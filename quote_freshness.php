...
function qa_quote_max_age(string $assetType, bool $strict = false): int {
  ...
  if ($assetType === 'crypto')      return $strict ? $age('QA_MAXAGE_CRYPTO_STRICT', 6)        : $age('QA_MAXAGE_CRYPTO', 12);
  if ($assetType === 'forex')       return $strict ? $age('QA_MAXAGE_FOREX_STRICT', 7200)      : $age('QA_MAXAGE_FOREX', 14400);
  if ($assetType === 'stocks')      return $strict ? $age('QA_MAXAGE_STOCKS_STRICT', 129600)   : $age('QA_MAXAGE_STOCKS', 172800);
  if ($assetType === 'arab')        return $strict ? $age('QA_MAXAGE_ARAB_STRICT', 129600)     : $age('QA_MAXAGE_ARAB', 172800);
