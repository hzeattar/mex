import{j as k,k as u,h as f,t as n,m as g,g as l,l as J,c as E,s as Z,n as ee,i as _e}from"./main-BjF5hH3U.js";const I=globalThis.__MEX_FUNDING_CACHE__||(globalThis.__MEX_FUNDING_CACHE__=new Map),be=45e3;let C=[];const te=[{key:"deposit",label:"funding.deposit",fallback:"Deposit",icon:u.deposit},{key:"withdraw",label:"funding.withdraw",fallback:"Withdraw",icon:u.withdraw},{key:"history",label:"funding.history",fallback:"History",icon:u.wallet}];function Xe(e={}){const t=ue(e),r=k("wallet")||{};k("kyc");const i=k("level")||{},s=k("mode")==="real"?"real":"demo",a=s==="real"?r.real||{}:r.demo||{};return i.current,`
    ...
    </section>`}async function A(e,t={},r=be){const i=e,s=Date.now(),a=I.get(i);if(a&&s-a.time<r)return a.data;const o=await E(e,t);return I.set(i,{time:s,data:o}),o}async function ke(e,t){try{const r=k("mode")==="real"?"real":"both",i=await A(`/payment_methods/list.php?kind=${encodeURIComponent(t)}&scope=${r}&currency=*`,{timeout:9e3,retry:1},9e4),s=i?.items||[],a=Se(i?.categories||[],s);e.__fundingKind=t,e.__fundingMethods=s,e.__fundingCategories=a,e.__fundingCategory=a[0]?.key||"",e.__fundingSelectedMethodId="",e.__fundingBonuses={},await Promise.all(a.map(async o=>{try{const d=await E(`/wallet/bonuses.php?method_key=${encodeURIComponent(o.key)}`,{timeout:4e3});d?.ok&&d.bonus&&(e.__fundingBonuses[o.key]=d.bonus)}catch{}})),ne(e),ie(e),S(e)}catch{const i=e.querySelector("#funding-categories"),s=e.querySelector("#method-cards");i&&(i.innerHTML=`<div class="empty-state empty-state--compact">${n("funding.sections_unavailable","Payment sections are temporarily unavailable.")}</div>`),s&&(s.innerHTML="")}}function Se(e,t){const r=new Set(t.map(x).filter(Boolean)),i=[];e.forEach(a=>{const o=q(a.key||a.key_slug||a.label);!o||!r.has(o)||i.push({key:o,label:a.label||T(o),hint:a.hint||X(o),icon:a.image_url?`<img src="${f(a.image_url)}" alt="">`:B(o,a.icon)})}),r.forEach(a=>{i.some(o=>o.key===a)||i.push({key:a,label:T(a),hint:X(a),icon:B(a)})});const s={card:1,bank:2,crypto:3,crypto_bot:4,manual:99};return i.sort((a,o)=>(s[a.key]||99)-(s[o.key]||99)),i}function ne(e){const t=e.querySelector("#funding-categories"),r=e.__fundingCategories||[],i=e.__fundingCategory||r[0]?.key||"";if(t){if(!r.length){t.innerHTML=`<div class="empty-state empty-state--compact">${n("funding.no_sections","No active funding sections are configured by admin.")}</div>`;return}t.innerHTML=r.map(s=>{const o=e.__fundingBonuses?.[s.key]||(s.key==="crypto"?{amount:10}:null),d=o?`
    ...
    <button type="button" class="${s.key===i?"active":""}" data-funding-category="${f(s.key)}">
      ...
      </div>`;return}const o=!s&&r>0,d=Math.max(1,Number(t.expires_hours||24)),c=o?ze(t,r,d):0;a.className=`deposit-transfer-panel is-ready${o?"":" is-preview"}`,a.innerHTML=`
    <div class="deposit-transfer-ready">
      <div class="deposit-transfer-title">
        <span>${u.deposit}</span>
        <div>
  ...
  </div>`}function se(e){const t=String(e?.payment_qr_url||"").trim();if(t)return t;const r=String(e?.payment_address||"").trim();return r?`https://api.qrserver.com/v1/create-qr-code/?size=280x280&margin=12&data=${encodeURIComponent(r)}`:""}function qe(e){if(j(e))return"";const t=String(e?.payment_address||"").trim(),r=ae(e?.fields||{});return t||r.length?"":`<div class="funding-method-warning">${u.lock}<div><strong>${n("funding.route_unavailable","Funding route unavailable")}</strong><small>${n("funding.choose_another_method","Choose another method or try again shortly.")}</small></div></div>`}function ae(e){const t=[];return Array.isArray(e)?e.forEach(r=>{if(!r)return;const i=r.label||r.name||r.key||"",s=r.value||r.default||r.text||"";i&&s&&t.push({label:i,value:s})}):e&&typeof e=="object"&&Object.entries(e).forEach(([r,i])=>{if(!(i==null||i===""))if(typeof i=="object"){const s=i.label||i.name||r,a=i.value||i.text||i.default||"";a&&t.push({label:s,value:a})}else t.push({label:Qe(r),value:i})}),t}function Te(e){return Array.isArray(e)?e.filter(Boolean).map(t=>(t.value||t.default||t.text)&&!t.collect?null:t).filter(Boolean):!e||typeof e!="object"?[]:Object.entries(e).map(([t,r])=>r&&typeof r=="object"?(r.value||r.default||r.text)&&!r.collect?null:{name:t,...r}:null).filter(Boolean)}async function Le(e,t,r){if(e.preventDefault(),t.__fundingSubmitting)return;const i=e.target,s=t.querySelector("#form-status"),a=t.querySelector("#funding-submit"),o=new FormData(i),d=le(t),c=r==="deposit",h=Number(o.get("amount")||0),D=String(o.get("notes")||"").trim(),y=d?j(d):!1;try{if(k("mode")!=="real")return m(s,n("funding.switch_real_before_submit","Switch to Real before submitting live funding requests."),"error");if(!d)return m(s,n("funding.select_active_method_first","Select an active payment method first."),"error");const _=ce(d,h);if(_)return m(s,_,"error");const v=i.querySelector('input[name="proof"]')?.files?.[0]||null;if(c&&!y&&d.proof_required&&!v)return m(s,n("funding.upload_proof_before_confirm","Upload transfer proof before confirming."),"error");const p=Be(t),$=p.destination||p.wallet_address||p.bank_account||p.iban||p.address||D;if(!c&&!String($||"").trim())return m(s,n("funding.enter_payout_destination","Enter payout destination details."),"error");t.__fundingSubmitting=!0,a&&(a.disabled=!0,a.dataset.originalText=a.dataset.originalText||a.textContent||"",a.textContent=c?y?n("funding.opening_checkout","Opening checkout..."):n("funding.confirming","Confirming..."):n("funding.submitting","Submitting...")),m(s,y?n("funding.opening_secure_checkout","Opening secure checkout..."):c?n("funding.sending_transfer_confirmation","Sending transfer confirmation..."):n("funding.sending_payout_request","Sending payout request..."),"info");const U=d.code||d.method||d.id||"",P=d.currency||"USDT",he=t.__fundingCategory||x(d),R=t.querySelector("[data-deposit-deadline]")?.dataset.depositDeadline||"",V={notes:D,category:he,method_title:d.title||d.name||d.code||"",client_deadline_at:R?Math.floor(Number(R)/1e3):null,proof_attached:!!v,fields:p},me=y?"/deposits/stripe_checkout.php":c?"/deposits/create.php":"/withdrawals/create.php",ye=c?{provider:d.provider||"",method:U,currency:P,amount:h,details:V}:{method:U,currency:P,amount:h,destination:$,details:{...V,destination:$}},b=await J(me,ye,{timeout:y?18e3:14e3,headers:{"Idempotency-Key":Re(c?"dep":"wd")}});if(!b||b.ok===!1)return m(s,b?.error||n("common.request_failed","Request failed"),"error");if(y&&b.checkout_url){m(s,n("funding.redirecting_checkout","Redirecting to checkout..."),"success"),window.location.assign(b.checkout_url);return}if(c&&v){const W=b.deposit?.id||b.id||b.deposit_id||null;if(W){const H=new FormData;H.append("deposit_id",String(W)),H.append("proof",v),await _e("/deposits/upload_proof.php",H,{timeout:18e3})}}m(s,c?n("funding.transfer_confirmation_received","Transfer confirmation received. Your deposit is now being processed."):n("funding.withdrawal_request_received","Withdrawal request received. You can track it from history."),"success"),Ne(t,c,h,d),I.delete(c?"/deposits/list.php":"/withdrawals/list.php")}catch(_){m(s,_.message||n("common.request_failed","Request failed"),"error")}finally{t.__fundingSubmitting=!1,a&&(a.disabled=!1,a.textContent=a.dataset.originalText||(c?n("funding.confirm_transfer","Confirm transfer"):n("funding.submit_withdraw","Submit withdrawal request")))}}async function De(e){if(!e.querySelector("#funding-history-all"))return;const r=await Promise.allSettled([A("/deposits/list.php",{timeout:8e3,retry:1},2e4),A("/withdrawals/list.php",{timeout:8e3,retry:1},2e4)]);e.__fundingHistoryItems=[...r[0].status==="fulfilled"?(r[0].value?.items||[]).map(i=>G(i,"deposit")):[],...r[1].status==="fulfilled"?(r[1].value?.items||[]).map(i=>G(i,"withdraw")):[]].sort((i,s)=>s.sortTime-i.sortTime).slice(0,80),oe(e)}function oe(e){const t=e.querySelector("#funding-history-all");if(!t)return;const r=e.__fundingHistoryFilter||"all";e.querySelectorAll("[data-history-filter]").forEach(s=>s.classList.toggle("active",s.dataset.historyFilter===r));const i=(e.__fundingHistoryItems||[]).filter(s=>Ie(s,r));if(!i.length){t.innerHTML=`<div class="empty-state empty-state--compact">${n("funding.no_matching_history","No matching funding history yet.")}</div>`;return}t.innerHTML=`
    ...
    </div>`}function G(e,t){const r=Number(e.amount||0),i=e.created_at||e.updated_at||"",s=t==="ledger"?"posted":String(e.status||"pending").toLowerCase(),a=e.reference||e.ref||e.ref_id||e.txid||e.tx_hash||e.transaction_id||e.id||"",o=e.note||e.notes||e.memo||e.description||"";return{kind:t,amount:r,currency:e.currency||"USDT",status:s,method:We(t==="ledger"?e.type||e.ref_type||"ledger":e.method_label||e.provider||e.method_code||e.method||"manual"),created:i,reference:String(a||""),note:String(o||""),sortTime:ge(i)}}function He(e){const t=e.kind==="deposit"||e.kind==="ledger"&&e.amount>=0,r=[[n("funding.method","Method"),e.method],[n("funding.date","Date"),z(e.created)],[n("kyc.status","Status"),N(e.status)],e.reference?[n("funding.reference","Reference"),e.reference]:null,e.note?[n("funding.notes","Notes"),e.note]:null].filter(Boolean);return`<article class="funding-history-card funds-history-card ${t?"is-positive":"is-negative"}" data-funding-history-card>
    <div class="funding-history-main">
      <span class="history-kind ${t?"is-deposit":"is-withdraw"}">${e.kind==="withdraw"?u.withdraw:u.deposit}</span>
      <div><strong>${pe(e.kind)}</strong><small>${l(e.method)} - ${l(z(e.created))}</small></div>
    ...
    <div class="funds-history-amount"><strong>${t?"+":""}${g(e.amount)} ${l(e.currency)}</strong><span class="${fe(e.status)}">${l(N(e.status))}</span></div>
