import{i as o,e as s,t as r,c as w,b as f}from"./main-CO2_3DUR.js";let m=[];function k(){return`
    ...
    </div>`}function E(e){g(e),e.querySelector("#refresh-news")?.addEventListener("click",()=>g(e)),e.addEventListener("click",y),e.addEventListener("keydown",$)}async function g(e){try{const t=w(),a=await f(`/news/list.php?lang=${encodeURIComponent(t)}`,{timeout:6e3}),n=e.querySelector("#news-grid");if(!n||!a)return;const l=a.items||[];if(m=l,!l.length){n.innerHTML=`<p class="text-muted text-sm col-span-full text-center py-12">${s(r("news.empty"))}</p>`;return}n.innerHTML=l.map((c,d)=>v(c,d)).join(""),b(n)}catch(t){const a=e.querySelector("#news-grid");a&&(a.innerHTML=`<p class="text-red text-sm col-span-full text-center py-8">${s(t.message)}</p>`)}}function b(e){e.querySelectorAll("[data-news-index]").forEach(t=>{const a=n=>{n.stopPropagation(),p(Number(t.dataset.newsIndex||0))};t.addEventListener("click",a),t.addEventListener("keydown",n=>{n.key!=="Enter"&&n.key!==" "||(n.preventDefault(),a(n))})})}function v(e,t){const a=x(e),n=e.image_url?`<img src="${s(e.image_url)}" class="news-card-media" loading="lazy" alt="${s(a.title)}" />`:`<div class="news-card-media news-card-media--fallback">${o.news}</div>`,l=e.source||e.category||"MEX Group";return`<article class="news-card" data-news-index="${t}" role="button" tabindex="0" aria-label="${s(a.title)}">
    ${n}
  ...
  </article>`}function x(e){const a=w()==="ar",n=a&&(e.title_ar||e.titleAr)||e.title_en||e.title||e.headline||"MEX Group Update",l=a&&(e.body_ar||e.bodyAr)||e.body_en||e.body||e.excerpt||"",c=a&&(e.excerpt_ar||e.excerptAr)||e.excerpt_en||e.excerpt||l;return{title:n,body:l,excerpt:c}}function y(e){const t=e.target.closest("[data-news-index]");t&&p(Number(t.dataset.newsIndex||0))}function $(e){if(e.key!=="Enter"&&e.key!==" ")return;const t=e.target.closest("[data-news-index]");t&&(e.preventDefault(),p(Number(t.dataset.newsIndex||0)))}function p(e){const t=m[e];if(!t)return;const a=x(t),n=t.image_url?`<img src="${s(t.image_url)}" class="news-dialog-media" alt="${s(a.title)}" />`:`<div class="news-dialog-media news-dialog-media--fallback">${o.news}</div>`,l=t.source||t.category||"MEX Group",c=document.createElement("div");c.className="dialog-backdrop news-dialog-backdrop",c.innerHTML=`
