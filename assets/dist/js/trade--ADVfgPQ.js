import{s as k,g as o,i as Q,e as c,b as M,$ as d,c as O,j as f,_ as Tt,d as h,n as at,a as _,m as x,p as ot}from"./main-B7BYCc04.js";import{marketIconPath as Et,marketInitial as Mt}from"./marketIcon-D-Yq8Sis.js";let K=null,B=null,E=0,U=null;const Nt=6500;function Ut(t,e,s,a,n={}){if(A(),!t||!t.length)return A;const l=++E,r=bt(t,n.maxSymbols||18);return r.length&&(typeof EventSource=="function"?Ot(r,e,s,a,l,n):W(r,e,s,a,l,n)),A}function bt(t,e){const s=Math.max(1,Number(e||18));return[...new Set((t||[]).map(a=>String(a||"").toUpperCase()).filter(Boolean))].slice(0,s)}function Ot(t,e,s,a,n,l){R();const r="/api/stream/sse.php?symbols="+encodeURIComponent(t.join(","))+"&type="+encodeURIComponent(e)+"&scope=watchlist";let i=!1,u=setTimeout(()=>{n!==E||i||(R(),W(t,e,s,a,n,l))},Math.max(3e3,Number(l.fallbackAfter||5e3)));U=new EventSource(r,{withCredentials:!0}),U.addEventListener("open",()=>{i=!0,u&&(clearTimeout(u),u=null)}),U.addEventListener("message",p=>{if(n===E)try{const m=JSON.parse(p.data||"[]");Array.isArray(m)&&m.length&&s&&s(m)}catch{}}),U.addEventListener("reconnect",()=>{n===E&&(R(),W(t,e,s,a,n,l))}),U.addEventListener("error",p=>{n===E&&(R(),W(t,e,s,a,n,l))})}function W(t,e,s,a,n,l){ft();const r=Math.max(4e3,Number(l.interval||Nt)),i=bt(t,l.maxSymbols||18),u=async()=>{if(n===E){B=new AbortController;try{const p="/api/quotes.php?symbols="+encodeURIComponent(i.join(","))+"&type="+encodeURIComponent(e)+"&visible=1&purpose=watchlist&_="+Date.now(),m=await fetch(p,{credentials:"same-origin",cache:"no-store",signal:B.signal});if(n!==E)return;if(m.ok){const b=await m.json();n===E&&b&&b.items&&s&&s(b.items)}}catch(p){p.name}finally{n===E&&(K=setTimeout(u,r))}}};u()}function ft(){K&&(clearTimeout(K),K=null),B&&(B.abort(),B=null)}function R(){U&&(U.close(),U=null)}function A(){E+=1,R(),ft()}let T=null,H=null,rt=null,P=null,G=null,I=null,J=null,F=null,nt=null,y=null,$=0;const Pt=[{key:"favorites",label:"Favorites"},{key:"crypto",label:"Crypto"},{key:"forex",label:"Forex"},{key:"stocks",label:"Stocks"},{key:"commodities",label:"Commodities"},{key:"futures",label:"Futures"},{key:"arab",label:"Arab"}],jt=["1m","5m","15m","30m","1h","4h","1d"];function ce(t){t.symbol&&k("symbol",t.symbol.toUpperCase()),t.type&&k("type",t.type),t.tf&&k("tf",t.tf);const e=o("symbol"),s=o("type"),a=o("tf");return`<div class="trade-terminal flex flex-col lg:flex-row h-full mobile-pad">
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
      <div class="flex items-center justify-between px-2 lg:px-4 h-11 border-b border-line bg-surface shrink-0">
        ...
        <div class="flex gap-0.5 overflow-x-auto max-w-[46vw] lg:max-w-none" id="tf-bar">
          ${jt.map(n=>`<button class="px-1.5 py-1 text-[10px] rounded shrink-0 ${n===a?"bg-accent/20 text-accent":"text-muted hover:text-text"}" data-tf="${n}">${n}</button>`).join("")}
        </div>
      </div>
  ...
  </div>`}function Rt(t,e){return`<div class="fixed inset-0 z-[230] hidden lg:hidden" id="mobile-order-sheet">
