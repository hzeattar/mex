import{j as _,k as V,g as f,t as n,h as S,m as y,e as D,r as ie,c as x,s as N,S as de,L as J,b as X,p as Y,n as pe,o as G}from"./main-D1bSdNRv.js";import{marketIconPath as ue,marketInitial as me}from"./marketIcon-D-Yq8Sis.js";let T=null,M=[];const re=[{code:"USD",flag:"🇺🇸",name:"US Dollar"},{code:"EGP",flag:"🇪🇬",name:"Egyptian Pound"},{code:"SAR",flag:"🇸🇦",name:"Saudi Riyal"},{code:"AED",flag:"🇦🇪",name:"UAE Dirham"},{code:"KWD",flag:"🇰🇼",name:"Kuwaiti Dinar"},{code:"QAR",flag:"🇶🇦",name:"Qatari Riyal"},{code:"BHD",flag:"🇧🇭",name:"Bahraini Dinar"},{code:"OMR",flag:"🇴🇲",name:"Omani Rial"},{code:"JOD",flag:"🇯🇴",name:"Jordanian Dinar"},{code:"TRY",flag:"🇹🇷",name:"Turkish Lira"},{code:"EUR",flag:"🇪🇺",name:"Euro"},{code:"GBP",flag:"🇬🇧",name:"British Pound"}];function je(){const e=_("brand")||{},a=Q(),r=(_("portfolio")||{}).metrics||{},o=_("mode")==="real"?"real":"demo",s=_("level")||{},l=s.current||{},i=s.next||{},u=Number(s.confirmed_deposit_total||s.total_deposits||s.deposit_total||0),p=Number(i.min_deposit_total||i.min_total_deposit||i.required_deposit||0),d=Math.max(0,p-u),c=p>0?Math.min(100,Math.round(u/p*100)):u>0?100:0,h=Number(a.balance||a.available||0),v=Number(r.total_balance??h),m=Number(r.available_balance??a.available??v??0),b=Number(r.in_use_balance??a.holds??a.locked??0),$=Number(r.pnl_24_live??0),g=Number(r.pnl_total_live??0),k=l.name||l.name_en||l.level_code||"Starter";return i.name||i.name_en,`
    <div class="pro-dashboard animate-fade-in pro-dashboard-compact">
      <section class="pro-card pro-balance-overview" aria-label="Account overview">
        ...
        <div class="pro-balance-main-row">
          ...
          <div class="pro-balance-metrics">
            ...
            <span><small>${n("balance.available","Available balance")}</small><b id="home-balance-available">${y(m)}</b></span>
      ...
      <section class="pro-card pro-level-scroll-card">
        ...
        <div class="pro-level-rail" aria-label="Level benefits and next tiers">
          ${oe(s,{currentLevel:l,nextLevel:i,remainingToNext:d,nextRequired:p,levelProgress:c,mode:o})}
        </div>
      </section>
    ...
    </div>`}function Fe(e){E(e,_("portfolio")),requestAnimationFrame(()=>{const o=e.querySelector(".pro-level-rail");o&&ce(o,!1)});const a=o=>{if(o.target.tagName==="IMG"&&o.target.dataset.fallback==="initial"){o.target.style.display="none";const s=o.target.nextElementSibling;s&&(s.style.display="grid")}};e.addEventListener("error",a,!0),M.push(()=>e.removeEventListener("error",a,!0)),setTimeout(()=>{e.querySelectorAll("#home-markets > *").forEach((o,s)=>{o.classList.add("stagger-item"),o.style.animationDelay=`${s*.05+.1}s`})},50);const t=o=>{const s=o.target.closest("[data-home-lang-trigger]");if(s){o.preventDefault(),o.stopPropagation();const d=s.closest("[data-home-lang-menu]"),c=d?.querySelector("[data-home-lang-dropdown]"),h=d?.querySelector("[data-home-lang-overlay]"),v=c?.classList.toggle("hidden")===!1;h?.classList.toggle("hidden",!v),s.setAttribute("aria-expanded",v?"true":"false");return}const l=o.target.closest("[data-home-set-locale]");if(l){o.preventDefault(),o.stopPropagation();const d=l.dataset.homeSetLocale||l.dataset.lang;F(e),d&&d!==D()&&(l.disabled=!0,l.classList.add("is-loading"),ie(d));return}o.target.closest("[data-home-lang-menu]")||F(e);const i=o.target.closest("#home-pnl-chart-toggle");if(i){o.preventDefault();const c=e.querySelector("#home-pnl-chart-card")?.classList.toggle("hidden")===!1;i.setAttribute("aria-expanded",c?"true":"false");return}const u=o.target.closest("[data-home-tab]");if(!u)return;const p=u.dataset.homeTab;e.__homeMarketTab=p,e.querySelectorAll("[data-home-tab]").forEach(d=>{d.classList.toggle("active",d===u)}),O(e,p)};e.addEventListener("click",t),M.push(()=>e.removeEventListener("click",t));const r=o=>{const s=o.target.closest("#home-fx-select");if(!s)return;const l=A(s.value);localStorage.setItem("vp_home_currency",l),te(e,l)};e.addEventListener("change",r),M.push(()=>e.removeEventListener("change",r)),T&&document.removeEventListener("click",T),T=o=>{e.contains(o.target)||F(e)},document.addEventListener("click",T),Z(e),te(e,I()),e.__homeRefreshTimer=setInterval(()=>{Z(e)},8e3),M.push(()=>{e.__homeRefreshTimer&&(clearInterval(e.__homeRefreshTimer),e.__homeRefreshTimer=null)})}function Oe(){T&&(document.removeEventListener("click",T),T=null),M.forEach(e=>{try{e()}catch{}}),M=[]}async function Z(e){const a=_("mode")==="real"?"real":"demo";x("/wallet/summary.php",{timeout:0,retry:1,cacheTtl:6e3}).then(t=>{(t?.real||t?.demo)&&(N("wallet",{real:t.real||{},demo:t.demo||{}}),E(e))}).catch(()=>E(e)),x(`/user/level.php?lang=${encodeURIComponent(D())}`,{timeout:0,retry:1,cacheTtl:3e4}).then(t=>{t?.ok!==!1&&(t?.current||Array.isArray(t?.levels))&&(N("level",t),he(e))}).catch(()=>null),x("/markets.php?scope=home&supported=1&lite=1&with_quotes=1&no_rescue=1&limit=50",{timeout:0,retry:1,cacheTtl:15e3}).then(t=>{if(t&&Array.isArray(t.items)&&t.items.length){e.__homeAllMarkets=t.items;const r=e.__homeMarketTab||"all";O(e,r)}else throw new Error("empty_markets")}).catch(()=>{x("/markets.php?scope=home&supported=1&lite=1&limit=30",{timeout:8e3,retry:0,cacheTtl:3e4}).then(t=>{if(t&&Array.isArray(t.items)&&t.items.length)e.__homeAllMarkets=t.items,O(e,e.__homeMarketTab||"all");else{const r=e.querySelector("#home-markets");r&&(r.innerHTML=`<p class="pro-empty">${n("market.reconnecting","Markets are reconnecting...")}</p>`)}}).catch(()=>{const t=e.querySelector("#home-markets");t&&(t.innerHTML=`<p class="pro-empty">${n("market.reconnecting","Markets are reconnecting...")}</p>`)})}),x(`/trade/portfolio.php?fast=1&mode=${a}`,{timeout:0,retry:1,cacheTtl:5e3}).then(t=>{t?.ok!==!1&&(N("portfolio",t),E(e,t))}).catch(()=>E(e)),Promise.allSettled([a==="real"?x(`/signals.php?bot=1&home=1&fast=1&lang=${encodeURIComponent(D())}`,{timeout:0,retry:1,cacheTtl:12e3}).catch(()=>null):Promise.resolve(null),x(`/invest/plans.php?kind=contract&home=1&lang=${encodeURIComponent(D())}`,{timeout:0,retry:1,cacheTtl:3e4}).catch(()=>null)]).then(([t,r])=>{const o=t.status==="fulfilled"?t.value:null,s=r.status==="fulfilled"?r.value:null;ee(e,o?.items||[],s?.items||[])}).catch(()=>ee(e,[],[]))}function E(e,a=_("portfolio")){const t=Q();_("mode");const r=a?.metrics||{},o=Number(t.balance||t.available||0),s=Number(r.total_balance??o),l=Number(r.available_balance??t.available??o??0),i=Number(r.in_use_balance??t.holds??t.locked??0),u=Number(r.pnl_24_live??0),p=Number(r.pnl_total_live??0);e.__homeBalanceValues={total:s,available:l,inUse:i,pnl24:u,pnlTotal:p,currency:t.currency||"USDT"},R(e)}function R(e){const a=e.__homeBalanceValues||{},t=Number(a.total||0),r=Number(a.available||0),o=Number(a.inUse||0),s=Number(a.pnl24||0),l=Number(a.pnlTotal||0),i=a.currency||Q().currency||"USDT",u=Le(),p=A(u.code),d=Number(u.rate||0)>0?Number(u.rate):1,c=B=>Number(B||0)*d,h=p==="USD"?i:p,v=e.querySelector("#home-balance-total");v&&(v.dataset.countTo=String(t),v.textContent=y(c(t),L(p)));const m=e.querySelector("#home-balance-currency");m&&(m.textContent=p==="USD"?i:`${p} - base ${i}`);const b=e.querySelector("#home-balance-available");b&&(b.textContent=`${y(c(r),L(p))} ${h}`);const $=e.querySelector("#home-balance-inuse");$&&($.textContent=`${y(c(o),L(p))} ${h}`);const g=e.querySelector("#home-pnl-24");g&&(g.textContent=`${y(c(s),L(p))} ${h}`,g.className=s>=0?"text-buy":"text-sell");const k=e.querySelector("#home-pnl-total");k&&(k.textContent=`${y(c(l),L(p))} ${h}`,k.className=l>=0?"text-buy":"text-sell");const C=e.querySelector("#home-fx-total");C&&(C.textContent=`${y(c(t),L(p))} ${h}`);const q=e.querySelector("#home-fx-rate");q&&(q.textContent=p==="USD"?`1 ${i} = 1 USD`:`1 ${i} ~= ${y(d,L(p))} ${p}`);const w=e.querySelector("#home-fx-note");if(w){const B=u.source?` - ${u.source}`:"";w.textContent=p==="USD"?n("home.base_currency_note","Showing account base currency"):`${n("home.converted_estimate","Converted estimate")}${B}`,w.classList.toggle("is-error",!!u.error)}const U=e.querySelector("#home-fx-select");U&&U.value!==p&&(U.value=p),Te(e,c(l),c(s),h)}function he(e){const a=_("level")||{},t=a.current||{},r=a.next||{},o=Number(a.confirmed_deposit_total||a.total_deposits||a.deposit_total||0),s=Number(r.min_deposit_total||r.min_total_deposit||r.required_deposit||0),l=Math.max(0,s-o),i=s>0?Math.min(100,Math.round(o/s*100)):o>0?100:0,u=e.querySelector(".pro-level-scroll-card .pro-section-head h2");u&&(u.textContent=`${t.name||t.name_en||t.level_code||"Starter"} ${n("level.progress_suffix","progress")}`);const p=e.querySelector("#home-level-percent");p&&(p.textContent=`${i}%`);const d=e.querySelector("#home-level-progress");d&&(d.style.width=`${i}%`);const c=e.querySelector(".pro-level-rail");c&&(c.innerHTML=oe(a,{currentLevel:t,nextLevel:r,remainingToNext:l,nextRequired:s,levelProgress:i,mode:_("mode")==="real"?"real":"demo"}),requestAnimationFrame(()=>ce(c,!0)))}function ee(e,a=[],t=[]){const r=e.querySelector("#home-copy-section");if(!r)return;const o=_("mode")==="real"?"real":"demo",s=(a||[]).slice(0,6).map(ve).join(""),l=fe(t||[]);r.innerHTML=`
    ...
    <a href="#/invest" class="btn-primary btn-sm w-full mt-3">${n("bot.open_copy_desk","Open copy desk")}</a>
      ...
      <small>${l>0?`${l} ${n("earn.day_term","day term")}`:n("earn.flexible_term","Flexible term")} - ${n("level.label","Level")}: ${f(o)}</small>
  ...
  `).join(""),t.querySelectorAll("[data-symbol]").forEach(r=>{r.addEventListener("click",()=>pe("trade",{symbol:r.dataset.symbol,type:r.dataset.type}))}))}function O(e,a="all"){const t=Array.isArray(e.__homeAllMarkets)?e.__homeAllMarkets:[],r=String(a||"all").toLowerCase(),o=r==="commodity"?"commodities":r,l=(o==="all"?t:t.filter(u=>ge(u.type)===o)).slice(0,o==="all"?16:30),i=e.querySelector("#home-markets");if(!l.length){i&&(i.innerHTML=`<p class="pro-empty">${n("market.no_markets_available","No markets available in this desk.")}</p>`);return}ye(e,l),_e(e,l)}function ge(e){const a=String(e||"crypto").toLowerCase();return a==="commodity"?"commodities":a==="stock"?"stocks":a==="future"?"futures":a==="fx"?"forex":a}async function _e(e,a){const t=(a||[]).filter(s=>{if(!(Number(s?.price||s?.q_price||0)>0))return!0;const i=W(s);return i==="stale"||i==="unavailable"});if(!t.length)return;const r=new Map;t.forEach(s=>{const l=String(s.symbol||"").toUpperCase(),i=s.type||"crypto";l&&(r.has(i)||r.set(i,[]),r.get(i).push(l))});const o=[];await Promise.all([...r.entries()].map(async([s,l])=>{const i=[...new Set(l)],u=s==="crypto"?30:8,p=new Set;for(let d=0;d<i.length;d+=u){const c=i.slice(d,d+u),h=await x(`/quotes.php?symbols=${encodeURIComponent(c.join(","))}&type=${encodeURIComponent(s)}&visible=1&purpose=watchlist&_=${Date.now()}`,{timeout:s==="crypto"?2600:3400,cacheTtl:500,cache:"no-store"}).catch(()=>null);h?.items?.length&&h.items.forEach(v=>{p.add(String(v.symbol||"").toUpperCase()),z(e,v,s)})}l.forEach(d=>{const c=K(e,d),h=c&&c.querySelector("[data-home-price]")?.textContent!=="--";(!p.has(d)||!h)&&o.push({symbol:d,type:s})})})),$e(e,o)}async function $e(e,a){const t=new Map;a.forEach(r=>{!r.symbol||!r.type||(t.has(r.type)||t.set(r.type,[]),t.get(r.type).push(r.symbol))});for(const[r,o]of t.entries()){const s=r==="crypto"?12:2,l=r==="crypto"?12:4,i=[...new Set(o)].slice(0,l),u=[];for(let c=0;c<i.length;c+=s){const h=i.slice(c,c+s),v=await x(`/quotes.php?symbols=${encodeURIComponent(h.join(","))}&type=${encodeURIComponent(r)}&visible=1&purpose=watchlist&_=${Date.now()}`,{timeout:r==="crypto"?2600:3400,cacheTtl:500,cache:"no-store"}).catch(()=>null);v?.items?.length&&v.items.forEach(m=>z(e,m,r)),h.forEach(m=>{const b=K(e,m);b&&b.querySelector("[data-home-price]")?.textContent!=="--"||u.push(m)})}const p=r==="crypto"?6:2,d=[...new Set(u)].slice(0,r==="crypto"?6:2);for(let c=0;c<d.length;c+=p){const h=d.slice(c,c+p),v=await x(`/quotes.php?symbols=${encodeURIComponent(h.join(","))}&type=${encodeURIComponent(r)}&visible=1&purpose=watchlist&_=${Date.now()}`,{timeout:r==="crypto"?2600:3400,cacheTtl:500,cache:"no-store"}).catch(()=>null);v?.items?.length&&v.items.forEach(m=>z(e,m,r))}}}function z(e,a,t){const r=String(a.symbol||"").toUpperCase(),o=K(e,r);if(!o)return;const s=o.dataset.type||a.type||t,l=Number(a.price||a.q_price||0),i=Number(a.change_pct||a.q_change||0),u=o.querySelector("[data-home-price]"),p=o.querySelector("[data-home-change]"),d=o.querySelector("[data-home-source]");if(u&&l>0){const h=parseFloat(u.dataset.price||"0");if(u.textContent=G(l,s),u.dataset.price=String(l),h>0&&l!==h){const v=l>h?"animate-price-up":"animate-price-down";u.classList.remove("animate-price-up","animate-price-down"),requestAnimationFrame(()=>{u.classList.add(v),setTimeout(()=>u.classList.remove(v),800)})}}p&&(p.textContent=Y(i),p.className=`text-[11px] ${i>=0?"text-buy":"text-sell"}`),d&&l>0&&(d.textContent=P(a));const c=o.querySelector("[data-quote-chip]");c&&l>0&&(c.className=`status-chip ${se(a)}`,c.textContent=P(a))}function K(e,a){return[...e.querySelectorAll("[data-symbol]")].find(t=>String(t.dataset.symbol||"").toUpperCase()===String(a||"").toUpperCase())}function xe(e,a){const t=e.symbol||"--";return`<span class="${a}">
    <img src="${S(ue(e,e.type||"crypto"))}" alt="${S(t)}" loading="lazy" data-fallback="initial" />
    <b style="display:none">${f(me(t))}</b>
  </span>`}function oe(e,a){const t=Array.isArray(e?.levels)?e.levels:[],r=a.currentLevel?.id,o=String(a.currentLevel?.level_code||"").toLowerCase(),s=H(a.currentLevel),l=t.length?t:[a.currentLevel,a.nextLevel].filter(Boolean),i=new Map,u=new Set,p=m=>{if(!m||typeof m!="object")return;const b=H(m);b&&u.has(b)||(b&&u.add(b),i.set(b||`level-${i.size}`,m))};l.forEach(p),p(a.currentLevel),p(a.nextLevel);const d=Array.from(i.values()).sort((m,b)=>{const $=j(m)-j(b);return Math.abs($)>1e-7?$:Number(m.sort_order||0)-Number(b.sort_order||0)}),c=d.findIndex(m=>{if(!m||typeof m!="object")return!1;const b=r&&Number(m.id)===Number(r),$=o&&String(m.level_code||"").toLowerCase()===o,g=s&&H(m)===s;return!!(b||$||g)}),h=[],v=[];for(let m=0;m<c;m++)v.push({lvl:d[m],index:m,rel:"before"});c>=0&&v.push({lvl:d[c],index:c,rel:"current"});for(let m=c+1;m<d.length;m++)v.push({lvl:d[m],index:m,rel:"after"});return v.forEach(({lvl:m,index:b,rel:$})=>{if(!m||typeof m!="object")return;const g=$==="current",k=c>=0&&b<c,C=j(m),q=g?a.levelProgress:k?100:0,w=m.name||m.name_en||m.level_code||`Tier ${b+1}`,U=a.nextLevel?.name||a.nextLevel?.name_en||"next tier";h.push(ke({label:g?n("level.current_tier","Current tier"):k?n("level.completed","Completed tier"):n("level.locked","Locked tier"),title:w,sub:g?a.nextRequired>0?`${y(a.remainingToNext)} USDT ${n("level.to_next","to")} ${U}`:n("level.top_active","Top tier permissions are active"):k?n("level.unlocked_completed","Unlocked and completed"):C>0?`${y(C)} USDT ${n("level.required","required")}`:n("level.starter_access","Starter access"),progress:q,state:g?"current":k?"completed":"locked",isCurrent:g,perks:Se(m,we(w,g,a.mode)),features:m.features||{}}))}),h.join("")}function H(e){return String(e?.id||e?.level_code||e?.name||e?.name_en||"").toLowerCase()}function j(e){return Number(e?.min_deposit_total||e?.min_total_deposit||e?.required_deposit||0)}function ke({label:e,title:a,sub:t,progress:r,state:o,perks:s,isCurrent:l,features:i}){const u=r==null?null:Math.max(0,Math.min(100,Number(r)||0)),p=Array.isArray(s)?s:[String(s||"")].filter(Boolean),d=i||{},c=[];return d.trading&&c.push({key:"trading",icon:"📈",label:n("feat.trading","Trading")}),d.copy_bot&&c.push({key:"copy_bot",icon:"🤖",label:n("feat.copy_bot","Copy Bot")}),d.contracts&&c.push({key:"contracts",icon:"📋",label:n("feat.contracts","Contracts")}),d.support&&c.push({key:"support",icon:"🎧",label:n("feat.support","Support")}),d.portfolio_manager&&c.push({key:"portfolio_manager",icon:"👔",label:n("feat.portfolio_manager","Manager")}),`<article class="pro-level-rail-card is-${S(o)}"${l?' data-current-level-card="1"':""}>
    <div class="pro-level-card-row"><span>${f(e)}</span>${u==null?"":`<b>${u}%</b>`}</div>
    <strong>${f(a)}</strong>
    <small>${f(t||"")}</small>
    ${u==null?"":`<div class="pro-mini-progress"><i style="width:${u}%"></i></div>`}
    ${c.length?`<div class="pro-level-features">${c.map(h=>`<span class="pro-level-feat ${h.key}" title="${f(h.label)}">${h.icon} <small>${f(h.label)}</small></span>`).join("")}</div>`:""}
    <ul class="pro-level-benefits">
      ${p.slice(0,5).map(h=>`<li>${f(h)}</li>`).join("")}
    </ul>
  </article>`}function Se(e,a){const t=e?.perks||e?.perks_en||e?.features||e?.description||"",r=Array.isArray(a)?a:String(a||"").split("|");if(!t)return r.filter(Boolean);const o=String(t).replace(/<[^>]+>/g," ").replace(/\s+/g," ").trim(),s=o.split(/[\n\r,;|•·]+/).map(l=>l.trim()).filter(Boolean);return(s.length>1?s:[o]).slice(0,5)}function we(e,a,t){const r=String(e||"").toLowerCase();return r.includes("vip")?["Priority account handling","Highest contract access","Premium copy desk limits"]:r.includes("platinum")?["Advanced copy desk access","Higher contract caps","Priority funding review"]:r.includes("gold")?["Premium copy desk","Higher contract caps","Dedicated account guidance"]:a&&t==="real"?["Live trading enabled","Wallet and funding access","Copy desk eligibility"]:["Market access","Watchlist and trade desk","Level progression benefits"]}function Ce(){const e=I(),a=re.map(t=>`
  ...
  </div>`}function I(){return A(localStorage.getItem("vp_home_currency")||"USD")}function A(e){const a=String(e||"USD").toUpperCase().replace(/[^A-Z]/g,"").slice(0,3);return re.some(t=>t.code===a)?a:"USD"}function L(e){return["JPY"].includes(e)?0:2}function Le(){const e=I(),a=_("home.fx")||{};return A(a.code)===e&&Number(a.rate||0)>0?{code:e,rate:Number(a.rate),source:a.source||"",error:!!a.error}:{code:e,rate:1,source:e==="USD"?"base":"",error:!1}}async function te(e,a=I()){const t=A(a);if(localStorage.setItem("vp_home_currency",t),t==="USD"){N("home.fx",{code:"USD",rate:1,source:"base",updated_at:Math.floor(Date.now()/1e3)}),R(e);return}const r=_("home.fx")||{};A(r.code)===t&&Number(r.rate||0)>0&&R(e);try{const o=await x(`/fx_rate.php?to=${encodeURIComponent(t)}`,{timeout:4e3,retry:1,cacheTtl:36e5}),s=Number(o?.rate||0);s>0?N("home.fx",{code:t,rate:s,source:o.source||"fx",updated_at:Number(o.updated_at||0),error:!1}):N("home.fx",{code:t,rate:Number(r.rate||1),source:o?.source||"fx",updated_at:Number(o?.updated_at||0),error:!0})}catch{N("home.fx",{code:t,rate:Number(r.rate||1),source:r.source||"cached",updated_at:Number(r.updated_at||0),error:!0})}R(e)}function Ne(){const e=D(),a=de.map(t=>`<button type="button" class="home-lang-option${t===e?" is-active":""}" data-home-set-locale="${S(t)}" data-lang="${S(t)}">
  ...
  </div>`}function F(e){const a=e.querySelector("[data-home-lang-dropdown]"),t=e.querySelector("[data-home-lang-overlay]"),r=e.querySelector("[data-home-lang-trigger]");a&&a.classList.add("hidden"),t&&t.classList.add("hidden"),r&&r.setAttribute("aria-expanded","false")}function Te(e,a,t,r=""){const o=e.querySelector("#home-pnl-chart-value"),s=e.querySelector("#home-pnl-spark");o&&(o.textContent=`${y(a)}${r?" "+r:""}`,o.className=a>=0?"text-buy":"text-sell"),s&&(s.innerHTML=ne(a,t))}function ne(e=0,a=0){const t=Number(e||0),r=Number(a||t||1),o=Array.from({length:12},(d,c)=>{const h=Math.sin((c+1)*.9)*Math.max(12,Math.abs(r)*.025);return t*(c/11)+h}),s=Math.min(...o,0),l=Math.max(...o,1),i=Math.max(1,l-s),u=o.map((d,c)=>{const h=5+c*10,v=54-(d-s)/i*44;return`${h.toFixed(1)},${v.toFixed(1)}`}).join(" "),p=t>=0?"var(--buy)":"var(--sell)";return`<svg viewBox="0 0 120 64" role="img" aria-label="PnL total chart" preserveAspectRatio="none">
