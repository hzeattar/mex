import{g as l,e as i,m as r,a as d,i as v,b as c,p as x,c as u,n as f}from"./main-BotwwwQy.js";import{m as y,a as g}from"./marketIcon-BqfrwX_4.js";function q(){const t=l("brand")||{},s=S(),a=l("mode");return`
    <div class="space-y-6 animate-fade-in">
      ...
      <section class="card bg-gradient-to-br from-panel-2 to-panel relative overflow-hidden">
        ...
        <div class="relative mt-4 grid grid-cols-2 sm:grid-cols-4 gap-3">
          ${o("Available",r(s.available),s.currency)}
          ${o("Balance",r(s.balance),s.currency)}
          ${o("Mode",a==="real"?"Real":"Demo",a==="real"?"Live":"Practice")}
    ...
    </div>`}function L(t){h(t)}async function h(t){try{const[s,a,e]=await Promise.all([d("/markets.php?type=crypto&lite=1&with_quotes=1",{timeout:7e3}),d("/trade/portfolio.php",{timeout:7e3}),d("/signals.php?bot=1&home=1&lang=en",{timeout:7e3}).catch(()=>null)]);s&&s.items&&k(t,s.items.slice(0,8)),a&&a.positions&&w(t,a.positions.slice(0,5)),e&&e.items&&e.items.length&&$(t,e.items)}catch{}}function $(t,s){const a=t.querySelector("#home-copy-section");a&&(a.innerHTML=`<div class="flex gap-3 overflow-x-auto pb-2 snap-x">${s.slice(0,6).map(e=>{const p=(e.direction||"BUY").toUpperCase(),m=e.symbol||e.market_symbol||"--";return`<div class="shrink-0 w-[280px] copy-card snap-start">
      <div class="flex items-center justify-between mb-2">
        <div class="flex items-center gap-2 min-w-0">
          ${b({symbol:m,type:e.type||e.market_type},"market-logo")}
          <div class="min-w-0">
            <strong class="text-xs truncate block">${i(m)}</strong>
            <span class="text-[10px] text-muted truncate block">${i(e.bot_name||e.bot_name_en||e.timeframe||"Copy signal")}</span>
          </div>
        </div>
        <span class="badge-${p==="BUY"?"buy":"sell"}">${p}</span>
      </div>
      <div class="copy-card__quote py-2">
        ...
        <strong>${Number(e.live_price||0)>0?"$"+r(e.live_price,e.type==="forex"?5:2):"--"}</strong>
        <span class="${Number(e.live_change_pct||0)>=0?"text-buy":"text-sell"}">${u(e.live_change_pct||0)}</span>
      </div>
    ...
    </div>`}).join("")}</div>`)}function k(t,s){const a=t.querySelector("#home-markets");a&&(a.innerHTML=s.map(e=>`
    <button class="home-market-card" data-symbol="${c(e.symbol)}" data-type="${c(e.type||"crypto")}">
      ${b(e,"home-market-icon")}
      ...
      <div class="text-right">
        <div class="text-sm font-mono font-semibold">${x(e.price||e.q_price,e.type)}</div>
        <div class="text-[11px] ${Number(e.change_pct||e.q_change||0)>=0?"text-green":"text-red"}">${u(e.change_pct||e.q_change||0)}</div>
      </div>
    </button>
  `).join(""),a.querySelectorAll("[data-symbol]").forEach(e=>{e.addEventListener("click",()=>f("trade",{symbol:e.dataset.symbol,type:e.dataset.type}))}))}function b(t,s){const a=t.symbol||"--";return`<span class="${s}">
    <img src="${c(y(t,t.type||"crypto"))}" alt="${c(a)}" loading="lazy" onerror="this.style.display='none';this.nextElementSibling.style.display='grid';" />
    <b>${i(g(a))}</b>
  </span>`}function w(t,s){const a=t.querySelector("#home-positions");if(a){if(!s.length){a.innerHTML='<p class="text-muted text-sm text-center py-6">No open positions</p>';return}a.innerHTML=`<div class="space-y-2">${s.map(_).join("")}</div>`}}function _(t){const s=Number(t.pnl||t.unrealized_pnl||0);return`<div class="flex items-center justify-between p-3 rounded-lg bg-panel-2/50 border border-line">
    <div><strong class="text-sm">${i(t.symbol)}</strong><span class="text-[11px] text-muted ml-2">${i(t.side||"BUY")}</span></div>
    <div class="text-right"><div class="text-sm font-mono ${s>=0?"text-green":"text-red"}">${r(s)}</div></div>
  </div>`}function o(t,s,a){return`<div class="p-3 rounded-lg bg-panel-2/60 border border-line/50">
    ...
    <span class="w-8 h-8 rounded-xl bg-gradient-to-br from-accent/20 to-green/10 grid place-items-center text-accent">${v[s]||""}</span>
    <span class="text-xs font-medium">${t}</span>
  </a>`}function A(t){return t!=="real"?`<div class="relative min-h-[180px]">
  ...
  </div>`}function S(){const t=l("wallet")||{};return l("mode")==="real"?t.real||{balance:0,available:0,currency:"USDT"}:t.demo||{balance:1e4,available:1e4,currency:"USDT_DEMO"}}export{L as mount,q as render};
