import{g,e as c,m as y,a as v,i as S,b as $,p as w,n as q,c as f}from"./main-CD3d5mYX.js";import{m as E,a as N}from"./marketIcon-BqfrwX_4.js";function V(){const e=g("brand")||{},s=Y(),a=g("mode");return`
    <div class="space-y-6 animate-fade-in">
      ...
      <section class="card">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          ${u("Deposit","deposit","#/deposit")}
          ${u("Withdraw","withdraw","#/withdraw")}
          ${u("KYC","kyc","#/kyc")}
          ${u("Earn","earn","#/invest")}
    ...
    </div>`}function F(e){U(e)}async function U(e){try{const[s,a,t]=await Promise.all([v("/markets.php?scope=home&supported=1&lite=1&with_quotes=1",{timeout:7e3}),v("/trade/portfolio.php",{timeout:7e3}),v("/signals.php?bot=1&home=1&lang=en",{timeout:7e3}).catch(()=>null)]);if(s&&s.items){const n=s.items.slice(0,12);T(e,n),j(e,n)}a&&a.positions&&R(e,a.positions.slice(0,5)),t&&t.items&&t.items.length&&A(e,t.items)}catch{}}function A(e,s){const a=e.querySelector("#home-copy-section");a&&(a.innerHTML=`<div class="flex gap-3 overflow-x-auto pb-2 snap-x">${s.slice(0,6).map(t=>{const n=(t.direction||"BUY").toUpperCase(),o=t.symbol||t.market_symbol||"--";return`<div class="shrink-0 w-[280px] copy-card snap-start">
      ...
      ${O(t)?'<p class="text-[10px] text-muted mt-2">Awaiting desk levels</p>':""}
    ...
    <button class="home-market-card" data-symbol="${$(t.symbol)}" data-type="${$(t.type||"crypto")}">
      ...
      <div class="flex-1 min-w-0">
        ...
        <div class="mini-spark" aria-hidden="true">${B(t.symbol)}</div>
      </div>
      <div class="text-right">
        <div class="text-sm font-mono font-semibold" data-home-price>${z(t.price||t.q_price,t.type)}</div>
  ...
  `).join(""),a.querySelectorAll("[data-symbol]").forEach(t=>{t.addEventListener("click",()=>q("trade",{symbol:t.dataset.symbol,type:t.dataset.type}))}))}async function j(e,s){const a=new Map;s.forEach(n=>{const o=String(n.symbol||"").toUpperCase(),r=n.type||"crypto";o&&(a.has(r)||a.set(r,[]),a.get(r).push(o))});const t=[];await Promise.all([...a.entries()].map(async([n,o])=>{const r=await v(`/quotes.php?symbols=${encodeURIComponent(o.join(","))}&type=${encodeURIComponent(n)}&visible=1&purpose=watchlist`,{timeout:6500}).catch(()=>null),l=new Set;r?.items?.length&&r.items.forEach(i=>{l.add(String(i.symbol||"").toUpperCase()),C(e,i,n)}),o.forEach(i=>{const d=L(e,i),p=d&&d.querySelector("[data-home-price]")?.textContent!=="--";(!l.has(i)||!p)&&t.push({symbol:i,type:n})})})),P(e,t)}async function P(e,s){const a=new Map;s.forEach(t=>{!t.symbol||!t.type||(a.has(t.type)||a.set(t.type,[]),a.get(t.type).push(t.symbol))});for(const[t,n]of a.entries()){const o=t==="crypto"?12:2,r=t==="crypto"?12:4,l=[...new Set(n)].slice(0,r);for(let i=0;i<l.length;i+=o){const d=l.slice(i,i+o),p=await v(`/quotes.php?symbols=${encodeURIComponent(d.join(","))}&type=${encodeURIComponent(t)}&cache_only=1&purpose=watchlist`,{timeout:3500}).catch(()=>null);p?.items?.length&&p.items.forEach(m=>C(e,m,t))}}}function C(e,s,a){const t=String(s.symbol||"").toUpperCase(),n=L(e,t);if(!n)return;const o=n.dataset.type||s.type||a,r=Number(s.price||s.q_price||0),l=Number(s.change_pct||s.q_change||0),i=n.querySelector("[data-home-price]"),d=n.querySelector("[data-home-change]"),p=n.querySelector("[data-home-source]");i&&r>0&&(i.textContent=f(r,o)),d&&(d.textContent=w(l),d.className=`text-[11px] ${l>=0?"text-green":"text-red"}`),p&&r>0&&(p.textContent=b(s));const m=n.querySelector("[data-quote-chip]");m&&r>0&&(m.className=`status-chip ${M(s)}`,m.textContent=b(s))}function L(e,s){return[...e.querySelectorAll("[data-symbol]")].find(a=>String(a.dataset.symbol||"").toUpperCase()===String(s||"").toUpperCase())}function _(e,s){const a=e.symbol||"--";return`<span class="${s}">
    ...
    <div class="flex items-center justify-between gap-3">
      <div class="flex items-center gap-2 min-w-0">
        ...
        <div class="min-w-0">
          ...
          <span class="text-[10px] text-muted">${c(e.market_type||e.order_type||"spot")} / ${c(H(e))}</span>
        </div>
      </div>
      <div class="text-right">
    ...
    <div class="position-metrics mt-3">
      ...
      ${h("Mark",n>0?f(n,t):"--")}
      ${h("Size",y(e.qty||e.amount||e.size||e.units||0))}
      ${h("Lev",`${e.leverage||1}x`)}
    </div>
  </div>`}function h(e,s){return`<span><small>${e}</small><strong>${s}</strong></span>`}function H(e){return String(e.side||"buy").toUpperCase()==="SELL"?"SELL":"BUY"}function x(e,s,a){return`<div class="p-3 rounded-lg bg-panel-2/60 border border-line/50">
  ...
  </div>`}function u(e,s,a){return`<a href="${a}" class="flex flex-col items-center gap-2 p-3 rounded-lg border border-line hover:border-accent/40 bg-panel-2/30 transition-colors">
  ...
  </a>`}function I(e){return e!=="real"?`<div class="relative min-h-[180px]">
  ...
  </div>`}function Y(){const e=g("wallet")||{};return g("mode")==="real"?e.real||{balance:0,available:0,currency:"USDT"}:e.demo||{balance:1e4,available:1e4,currency:"USDT_DEMO"}}function z(e,s){const a=Number(e||0);return a>0?f(a,s):"--"}function b(e){const s=String(e.source||e.provider||"").toLowerCase(),a=String(e.timing_class||"").toLowerCase();return Number(e.price||e.q_price||0)<=0?"Unavailable":a==="stale"||e.is_stale?"Stale":e.delayed||s.includes("yahoo")?"Delayed":s.includes("binance")||s.includes("stream")||a==="live"?"Live":s?s.replace(/_/g," "):"Cached"}function M(e){const s=b(e).toLowerCase();return s==="live"?"status-chip-live":s==="unavailable"?"status-chip-locked":"status-chip-delayed"}function Q(e){return`<span data-quote-chip class="status-chip ${M(e)}">${b(e)}</span>`}function B(e){const s=String(e||"").split("").reduce((a,t)=>a+t.charCodeAt(0),0);return Array.from({length:12},(a,t)=>`<i style="height:${18+(s+t*13)%26}px"></i>`).join("")}function k(e,s){const a=Number(e||0);return a>0?f(a,s):"--"}function O(e){return!(Number(e.entry||e.entry_price||0)>0)||!(Number(e.tp1||e.take_profit_1||e.take_profit||0)>0)||!(Number(e.sl||e.stop_loss||0)>0)}export{F as mount,V as render};
