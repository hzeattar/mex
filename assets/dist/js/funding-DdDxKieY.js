import{j as $,k as u,h as f,t as n,m as p,g as l,l as J,c as Z,s as ee,n as te,i as be}from"./main-F9VxIRQ0.js";const A=globalThis.__MEX_FUNDING_CACHE__||(globalThis.__MEX_FUNDING_CACHE__=new Map),we=45e3;let q=[];const ne=[{key:"deposit",label:"funding.deposit",fallback:"Deposit",icon:u.deposit},{key:"withdraw",label:"funding.withdraw",fallback:"Withdraw",icon:u.withdraw},{key:"history",label:"funding.history",fallback:"History",icon:u.wallet}];function Je(e={}){const t=fe(e),r=$("wallet")||{};$("kyc");const i=$("level")||{},s=$("mode")==="real"?"real":"demo",a=s==="real"?r.real||{}:r.demo||{};return i.current,`
    <div class="funds-workspace animate-fade-in" data-active-funding-tab="${f(t)}">
      ...
      ${t==="history"?ke():$e(t)}
    </div>`}function Ze(e,t={}){const r=fe(t);ve(e,t);const i=o=>{const d=o.target.closest("[data-funding-tab]");if(d){o.preventDefault(),te("wallet",{action:d.dataset.fundingTab||"deposit"});return}const c=o.target.closest("[data-funding-category]");if(c){e.__fundingCategory=c.dataset.fundingCategory||"",e.__fundingSelectedMethodId="",ie(e),re(e),k(e);return}const g=o.target.closest("[data-method]");if(g){e.__fundingSelectedMethodId=g.dataset.method||"",k(e);return}const M=o.target.closest("[data-history-filter]");if(M){e.__fundingHistoryFilter=M.dataset.historyFilter||"all",de(e);return}const h=o.target.closest("[data-funding-history-toggle]");if(h){const _=h.closest("[data-funding-history-card]");if(_){const y=!_.classList.contains("is-expanded");_.classList.toggle("is-expanded",y),h.setAttribute("aria-expanded",y?"true":"false")}return}const v=o.target.closest("[data-copy-address]");if(v){Pe(v);return}o.target.closest("[data-switch-real]")&&(localStorage.setItem("vp_mode","real"),ee("mode","real"),location.reload())};e.addEventListener("click",i),q.push(()=>e.removeEventListener("click",i));const s=o=>{o.target.matches('input[name="amount"], [data-withdraw-field]')&&k(e)};e.addEventListener("input",s),q.push(()=>e.removeEventListener("input",s));const a=o=>{const d=o.target.closest('input[name="proof"]');if(!d)return;const c=e.querySelector("#proof-file-name"),g=d.files?.[0];c&&(c.textContent=g?`${g.name} - ${Xe(g.size||0)}`:n("funding.upload_file_hint","Image or PDF up to 8MB"))};if(e.addEventListener("change",a),q.push(()=>e.removeEventListener("change",a)),e.querySelectorAll("[data-quick-amount]").forEach(o=>{o.addEventListener("click",()=>{const d=e.querySelector('input[name="amount"]');d&&(d.value=o.dataset.quickAmount||""),k(e)})}),r==="history"){He(e);return}Se(e,r),e.querySelector("#funding-form")?.addEventListener("submit",o=>De(o,e,r))}function ve(e,t){const r=t&&t.stripe;if(!r)return;try{window.history.replaceState(null,"",window.location.pathname+"#/wallet")}catch{}const i=document.createElement("div");if(i.className="stripe-return-banner",e.prepend(i),r!=="success"){i.classList.add("is-cancel"),i.innerHTML=`<div><strong>${n("funding.payment_canceled","Card payment was canceled.")}</strong><small>${n("funding.payment_canceled_copy","You can start a new deposit whenever you are ready.")}</small></div>`;return}const s=Number(t.deposit||0);i.classList.add("is-pending"),i.innerHTML=`<span class="stripe-return-spin"></span><div><strong>${n("funding.verifying_payment","Verifying card payment...")}</strong></div>`,(async()=>{let a="";if(s>0)for(let o=0;o<4;o++){try{const d=await J("/deposits/stripe_sync.php",{deposit_id:s},{timeout:15e3});if(a=String(d?.status||""),a==="confirmed"||a==="failed")break}catch{}await new Promise(d=>setTimeout(d,2500))}if(a==="confirmed"){i.className="stripe-return-banner is-success",i.innerHTML=`<div><strong>${n("funding.deposit_confirmed","Payment confirmed. Funds credited to your wallet.")}</strong></div>`;try{const o=await Z("/bootstrap.php");o&&o.wallet&&ee("wallet",o.wallet)}catch{}te("wallet",{action:"history"})}else a==="failed"?(i.className="stripe-return-banner is-cancel",i.innerHTML=`<div><strong>${n("funding.deposit_failed","Payment was not completed.")}</strong></div>`):(i.className="stripe-return-banner",i.innerHTML=`<div><strong>${n("funding.payment_processing","Payment is processing. Your balance will update shortly.")}</strong></div>`)})()}function et(){document.querySelectorAll(".funds-workspace").forEach(e=>H(e)),q.forEach(e=>{try{e()}catch{}}),q=[]}function $e(e){const t=e==="deposit";return`
      ...
      <div class="panel-headline-row">
        ...
        <div class="history-filter-rail">
          ${["all","deposit","withdraw","pending","completed","failed"].map((e,t)=>`
            <button type="button" class="${t===0?"active":""}" data-history-filter="${e}">${l(We(e))}</button>
          `).join("")}
        </div>
      </div>
  ...
  `).join("")}}function re(e){const t=e.querySelector("#method-cards"),r=le(e);if(t){if(H(e),!r.length){t.innerHTML=`<div class="empty-state empty-state--compact">${n("funding.no_methods_section","No active methods under this section.")}</div>`,e.__fundingSelectedMethodId="",k(e);return}r.some(i=>w(i)===e.__fundingSelectedMethodId)||(e.__fundingSelectedMethodId=w(r[0])),t.innerHTML=r.map(i=>{const s=w(i);return`<button type="button" class="method-card ${s===e.__fundingSelectedMethodId?"active":""}" data-method="${f(s)}">
      ...
      </div>`;return}const o=!s&&r>0,d=Math.max(1,Number(t.expires_hours||24)),c=o?Ee(t,r,d):0;a.className=`deposit-transfer-panel is-ready${o?"":" is-preview"}`,a.innerHTML=`
    <div class="deposit-transfer-ready">
      <div class="deposit-transfer-title">
        <span>${u.deposit}</span>
        <div>
          <strong>${o?`${n("funding.transfer","Transfer")} ${p(r)} ${l(t.currency||"USDT")}`:l(t.title||t.name||t.code||n("funding.payment_method","Payment method"))}</strong>
      ...
      ${o?`<div class="deposit-timer" data-deposit-deadline="${c}">
        ...
        <small>${n("funding.expires","Expires")} ${Ye(c)}</small>
      </div>`:""}
      ${qe(t)}
      ...
      ${Te(t)}
    </div>`,o&&je(e,c)}function K(e,t=!1){const r=e.max_amount?p(e.max_amount):n("funding.flexible","Flexible"),i=t?n("funding.not_needed","Not needed"):e.proof_required?n("funding.receipt_required","Receipt required"):n("funding.optional","Optional"),s=t?n("funding.checkout","Checkout"):`${Math.max(1,Number(e.expires_hours||24))}h`;return`<div class="funding-route-summary" aria-label="Selected funding route summary">
    ...
    </label>`;return}r.innerHTML=`<div class="withdraw-fields-grid">${s.map(Me).join("")}</div>`}function Me(e){const t=Ue(e.name||e.key||e.id||e.label||"destination"),r=e.label||e.title||x(t),i=e.placeholder||e.hint||"",s=e.required||e.is_required?"required":"",a=String(e.type||"text").toLowerCase(),o=Re(e.options||e.values||[]);if(a==="textarea")return`<label class="block sm:col-span-2"><span class="field-label">${l(r)}</span><textarea name="${f(t)}" data-withdraw-field class="input mt-1" rows="3" placeholder="${f(i)}" ${s}></textarea></label>`;if(a==="select"&&o.length)return`<label class="block"><span class="field-label">${l(r)}</span><select name="${f(t)}" data-withdraw-field class="input mt-1" ${s}>${o.map(c=>`<option value="${f(c.value)}">${l(c.label)}</option>`).join("")}</select></label>`;const d=["email","number","tel","url"].includes(a)?a:"text";return`<label class="block"><span class="field-label">${l(r)}</span><input type="${d}" name="${f(t)}" data-withdraw-field class="input mt-1" placeholder="${f(i)}" ${s}></label>`}function se(e,t){return`<div class="deposit-transfer-empty"><span>${u.deposit}</span><strong>${l(e)}</strong><small>${l(t)}</small></div>`}function Ce(e){const t=[];return String(e?.image_url||"").trim()&&t.push(n("funding.badge_logo","Logo")),ae(e)&&t.push(n("funding.badge_qr","QR")),(String(e?.payment_address||"").trim()||oe(e?.fields||{}).length)&&t.push(n("funding.badge_details","Details")),e?.proof_required&&t.push(n("funding.badge_receipt","Receipt")),t.length||t.push(n("funding.needs_setup","Needs setup")),t.slice(0,4).map(r=>`<b>${l(r)}</b>`).join("")}function qe(e){const t=ae(e),r=String(e?.payment_address||"").trim();return!t&&!r?"":`<div class="transfer-qr-card transfer-qr-card--centered">
  ...
  </div>`}function ae(e){const t=String(e?.payment_qr_url||"").trim();if(t)return t;const r=String(e?.payment_address||"").trim();return r?`https://api.qrserver.com/v1/create-qr-code/?size=280x280&margin=12&data=${encodeURIComponent(r)}`:""}function Te(e){if(j(e))return"";const t=String(e?.payment_address||"").trim(),r=oe(e?.fields||{});return t||r.length?"":`<div class="funding-method-warning">${u.lock}<div><strong>${n("funding.route_unavailable","Funding route unavailable")}</strong><small>${n("funding.choose_another_method","Choose another method or try again shortly.")}</small></div></div>`}function oe(e){const t=[];return Array.isArray(e)?e.forEach(r=>{if(!r)return;const i=r.label||r.name||r.key||"",s=r.value||r.default||r.text||"";i&&s&&t.push({label:i,value:s})}):e&&typeof e=="object"&&Object.entries(e).forEach(([r,i])=>{if(!(i==null||i===""))if(typeof i=="object"){const s=i.label||i.name||r,a=i.value||i.text||i.default||"";a&&t.push({label:s,value:a})}else t.push({label:Ke(r),value:i})}),t}function Le(e){return Array.isArray(e)?e.filter(Boolean).map(t=>(t.value||t.default||t.text)&&!t.collect?null:t).filter(Boolean):!e||typeof e!="object"?[]:Object.entries(e).map(([t,r])=>r&&typeof r=="object"?(r.value||r.default||r.text)&&!r.collect?null:{name:t,...r}:null).filter(Boolean)}async function De(e,t,r){if(e.preventDefault(),t.__fundingSubmitting)return;const i=e.target,s=t.querySelector("#form-status"),a=t.querySelector("#funding-submit"),o=new FormData(i),d=ce(t),c=r==="deposit",g=Number(o.get("amount")||0),M=String(o.get("notes")||"").trim(),h=d?j(d):!1;try{if($("mode")!=="real")return m(s,n("funding.switch_real_before_submit","Switch to Real before submitting live funding requests."),"error");if(!d)return m(s,n("funding.select_active_method_first","Select an active payment method first."),"error");const v=ue(d,g);if(v)return m(s,v,"error");const _=i.querySelector('input[name="proof"]')?.files?.[0]||null;if(c&&!h&&d.proof_required&&!_)return m(s,n("funding.upload_proof_before_confirm","Upload transfer proof before confirming."),"error");const y=Ne(t),F=y.destination||y.wallet_address||y.bank_account||y.iban||y.address||M;if(!c&&!String(F||"").trim())return m(s,n("funding.enter_payout_destination","Enter payout destination details."),"error");t.__fundingSubmitting=!0,a&&(a.disabled=!0,a.dataset.originalText=a.dataset.originalText||a.textContent||"",a.textContent=c?h?n("funding.opening_checkout","Opening checkout..."):n("funding.confirming","Confirming..."):n("funding.submitting","Submitting...")),m(s,h?n("funding.opening_secure_checkout","Opening secure checkout..."):c?n("funding.sending_transfer_confirmation","Sending transfer confirmation..."):n("funding.sending_payout_request","Sending payout request..."),"info");const R=d.code||d.method||d.id||"",U=d.currency||"USDT",me=t.__fundingCategory||S(d),P=t.querySelector("[data-deposit-deadline]")?.dataset.depositDeadline||"",V={notes:M,category:me,method_title:d.title||d.name||d.code||"",client_deadline_at:P?Math.floor(Number(P)/1e3):null,proof_attached:!!_,fields:y},ye=h?"/deposits/stripe_checkout.php":c?"/deposits/create.php":"/withdrawals/create.php",_e=c?{provider:d.provider||"",method:R,currency:U,amount:g,details:V}:{method:R,currency:U,amount:g,destination:F,details:{...V,destination:F}},b=await J(ye,_e,{timeout:h?18e3:14e3,headers:{"Idempotency-Key":Ve(c?"dep":"wd")}});if(!b||b.ok===!1)return m(s,b?.error||n("common.request_failed","Request failed"),"error");if(h&&b.checkout_url){m(s,n("funding.redirecting_checkout","Redirecting to checkout..."),"success"),window.location.assign(b.checkout_url);return}if(c&&_){const W=b.deposit?.id||b.id||b.deposit_id||null;if(W){const I=new FormData;I.append("deposit_id",String(W)),I.append("proof",_),await be("/deposits/upload_proof.php",I,{timeout:18e3})}}m(s,c?n("funding.transfer_confirmation_received","Transfer confirmation received. Your deposit is now being processed."):n("funding.withdrawal_request_received","Withdrawal request received. You can track it from history."),"success"),ze(t,c,g,d),A.delete(c?"/deposits/list.php":"/withdrawals/list.php")}catch(v){m(s,v.message||n("common.request_failed","Request failed"),"error")}finally{t.__fundingSubmitting=!1,a&&(a.disabled=!1,a.textContent=a.dataset.originalText||(c?n("funding.confirm_transfer","Confirm transfer"):n("funding.submit_withdraw","Submit withdrawal request")))}}async function He(e){if(!e.querySelector("#funding-history-all"))return;const r=await Promise.allSettled([B("/deposits/list.php",{timeout:8e3,retry:1},2e4),B("/withdrawals/list.php",{timeout:8e3,retry:1},2e4)]);e.__fundingHistoryItems=[...r[0].status==="fulfilled"?(r[0].value?.items||[]).map(i=>X(i,"deposit")):[],...r[1].status==="fulfilled"?(r[1].value?.items||[]).map(i=>X(i,"withdraw")):[]].sort((i,s)=>s.sortTime-i.sortTime).slice(0,80),de(e)}function de(e){const t=e.querySelector("#funding-history-all");if(!t)return;const r=e.__fundingHistoryFilter||"all";e.querySelectorAll("[data-history-filter]").forEach(s=>s.classList.toggle("active",s.dataset.historyFilter===r));const i=(e.__fundingHistoryItems||[]).filter(s=>Ae(s,r));if(!i.length){t.innerHTML=`<div class="empty-state empty-state--compact">${n("funding.no_matching_history","No matching funding history yet.")}</div>`;return}t.innerHTML=`
    <div class="ledger-mobile-list md:hidden">${i.map(Fe).join("")}</div>
    <div class="hidden md:block overflow-x-auto">
      <table class="funding-history-table">
        ...
        <tbody>${i.map(Ie).join("")}</tbody>
      </table>
    </div>`}function X(e,t){const r=Number(e.amount||0),i=e.created_at||e.updated_at||"",s=t==="ledger"?"posted":String(e.status||"pending").toLowerCase(),a=e.reference||e.ref||e.ref_id||e.txid||e.tx_hash||e.transaction_id||e.id||"",o=e.note||e.notes||e.memo||e.description||"";return{kind:t,amount:r,currency:e.currency||"USDT",status:s,method:Qe(t==="ledger"?e.type||e.ref_type||"ledger":e.method_label||e.provider||e.method_code||e.method||"manual"),created:i,reference:String(a||""),note:String(o||""),sortTime:he(i)}}function Fe(e){const t=e.kind==="deposit"||e.kind==="ledger"&&e.amount>=0,r=[[n("funding.method","Method"),e.method],[n("funding.date","Date"),E(e.created)],[n("kyc.status","Status"),z(e.status)],e.reference?[n("funding.reference","Reference"),e.reference]:null,e.note?[n("funding.notes","Notes"),e.note]:null].filter(Boolean);return`<article class="funding-history-card funds-history-card ${t?"is-positive":"is-negative"}" data-funding-history-card>
    <div class="funding-history-main">
      <span class="history-kind ${t?"is-deposit":"is-withdraw"}">${e.kind==="withdraw"?u.withdraw:u.deposit}</span>
      <div><strong>${ge(e.kind)}</strong><small>${l(e.method)} - ${l(E(e.created))}</small></div>
    ...
    <div class="funds-history-amount"><strong>${t?"+":""}${p(e.amount)} ${l(e.currency)}</strong><span class="${pe(e.status)}">${l(z(e.status))}</span></div>
