import{t,l as y,e as S,j as _,k as L,h as m,g as n,c as $,u as w}from"./main-DveN8zZZ.js";const T=["general","deposit","withdrawal","trading","account","complaint"];function h(r){return{general:t("support.reason.general","General question"),deposit:t("support.reason.deposit","Deposit issue"),withdrawal:t("support.reason.withdrawal","Withdrawal issue"),trading:t("support.reason.trading","Trading issue"),account:t("support.reason.account","Account / KYC"),complaint:t("support.reason.complaint","Complaint")}[r]||r}function v(r){const a=String(r||"").toLowerCase();return{open:t("support.status.open","Open"),pending:t("support.status.pending","Pending"),answered:t("support.status.answered","Answered"),resolved:t("support.status.resolved","Resolved"),closed:t("support.status.closed","Closed")}[a]||a}function q(r){const a=String(r||"").toLowerCase();return a==="resolved"||a==="closed"?"is-closed":a==="answered"?"is-answered":"is-open"}function j(){const r=_("brand")||{},a=_("support")||{},e=r.whatsapp_support_url||a.whatsapp_url||"",o=/^https?:\/\//i.test(String(e));return`
      ...
      <section class="card support-ticket-card">
        ...
        <form class="support-ticket-form" data-ticket-form>
          ...
          <label class="support-field">
            ...
            <input name="subject" class="input-base" maxlength="190" placeholder="${m(t("support.field.subject_ph","Short summary (optional)"))}" />
          </label>
          <label class="support-field">
            <span>${t("support.field.message","Message")}</span>
            <textarea name="message" class="input-base" rows="4" maxlength="4000" required placeholder="${m(t("support.field.message_ph","Describe your issue in detail..."))}"></textarea>
          </label>
    ...
    </div>`}function i(r,a,e="info"){r&&(r.textContent=a,r.className=`text-xs support-form-status is-${e}`)}async function g(r){const a=r.querySelector("[data-ticket-list]");if(!a)return;let e=null;try{e=await $("/support/list.php")}catch{}if(!e||e.ok===!1){a.innerHTML=`<p class="text-xs text-red-400">${n(e?.error||t("common.request_failed","Request failed"))}</p>`;return}const o=Array.isArray(e.items)?e.items:[];if(!o.length){a.innerHTML=`<p class="text-xs text-slate-400">${t("support.no_tickets","No tickets yet. Your requests will appear here.")}</p>`;return}a.innerHTML=o.map(s=>`
    ...
    </button>`).join(""),a.querySelectorAll("[data-ticket-id]").forEach(s=>{s.addEventListener("click",()=>b(r,Number(s.dataset.ticketId)))})}async function b(r,a){const e=r.querySelector("[data-ticket-detail]");if(!e||!a)return;e.hidden=!1,e.innerHTML=`<p class="text-xs text-slate-400">${t("common.loading","Loading...")}</p>`;let o=null;try{o=await $(`/support/get.php?id=${a}`)}catch{}if(!o||o.ok===!1){e.innerHTML=`<p class="text-xs text-red-400">${n(o?.error||t("common.request_failed","Request failed"))}</p>`;return}const s=o.ticket||{},c=Array.isArray(o.messages)?o.messages:[];e.innerHTML=`
    ...
    <form class="support-reply-form" data-reply-form>
      <textarea name="message" class="input-base" rows="2" maxlength="4000" required placeholder="${m(t("support.reply_ph","Write a reply..."))}"></textarea>
      <div class="support-form-foot">
