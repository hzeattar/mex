import{h as f,i as b,b as $,e as s,m as n,d as w,k as x,f as _,j as M}from"./main-DKrvKuS9.js";function C(){const t=f("mode");return`
    ...
    `,p.innerHTML=`
      ...
      ${y("Open PnL",n(i),i>=0?"Profit":"Loss",i>=0?"text-buy":"text-sell")}
      ${y("Positions",String(r.length),"Open trades")}
      ${y("Balance",n(o),f("mode")==="real"?"USDT":"USDT_DEMO")}`);const u=t.querySelector("#pos-count");u&&(u.textContent=`${r.length} open`);const d=t.querySelector("#positions-table");if(d){if(!r.length){d.innerHTML='<p class="text-muted text-sm text-center py-8">No open positions. Start trading to see them here.</p>';return}d.innerHTML=`
    ...
    </div>`,w(d,"[data-close-pos]","click",(g,L)=>O(L.dataset.closePos,t))}}function S(t){const e=Number(t.pnl||t.unrealized_pnl||0),r=t.asset_type||t.type||"crypto",o=Number(t.mark_price||t.current_price||t.price||0),i=t.position_id||t.id||"",m=String(t.symbol||"").replace("@R@",""),l=String(t.side||"buy").toUpperCase()==="SELL"?"SELL":"BUY",c=Number(t.entry_price||t.open_price||0),p=Number(t.qty||t.amount||t.size||t.units||0),u=Number(t.margin_initial||t.margin||t.initial_margin||t.used_margin||0),d=Number(t.leverage||1),g=t.opened_at_label||t.created_at||t.opened_at||t.updated_at||"";return{pnl:e,type:r,mark:o,id:i,symbol:m,side:l,entry:c,size:p,margin:u,leverage:d,opened:g}}function P(t){const{pnl:e,type:r,mark:o,id:i,symbol:m,side:l,entry:c,size:p,margin:u,leverage:d}=S(t);return`<tr class="border-b border-line/50 hover:bg-panel-2/30">
    ...
    <td class="py-2.5 text-right font-mono text-xs">${c>0?x(c,r):"--"}</td>
    <td class="py-2.5 text-right font-mono text-xs">${o>0?x(o,r):"--"}</td>
    <td class="py-2.5 text-right text-xs">${n(p)}</td>
    <td class="py-2.5 text-right font-mono text-xs">${s(String(d||1))}x</td>
    <td class="py-2.5 text-right text-xs">${n(u)}</td>
    <td class="py-2.5 text-right font-mono ${e>=0?"text-buy":"text-sell"}">${n(e)}</td>
    <td class="py-2.5 text-right px-2">${i?`<button class="btn-ghost btn-sm text-red" data-close-pos="${_(i)}">Close</button>`:""}</td>
  </tr>`}function T(t){const{pnl:e,type:r,mark:o,id:i,symbol:m,side:l,entry:c,size:p,margin:u,leverage:d,opened:g}=S(t);return`<article class="portfolio-position-card">
    <div class="flex items-start justify-between gap-3">
      ...
      <div class="text-right">
        ...
        <b class="${e>=0?"text-buy":"text-sell"}">${n(e)}</b>
      </div>
    </div>
    ...
    <div class="hidden md:block overflow-x-auto">
      <table class="w-full text-sm">
        ...
        <tbody>${e.map(i=>`<tr class="border-b border-line/50">
          ...
          <td class="py-2 text-right font-mono text-xs">${x(i.price||i.limit_price)}</td>
    ...
    <div class="portfolio-position-metrics">
      ${a("Price",x(t.price||t.limit_price))}
      ${a("Amount",n(t.amount))}
