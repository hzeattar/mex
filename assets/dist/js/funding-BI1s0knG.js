import{j as x,l as H,u as Y,v as n,k as p,h as c,m,g as l,o as se,c as R,s as ve,n as re,x as $e,i as ke}from"./main-CYQDWk0e.js";const P=globalThis.__MEX_FUNDING_CACHE__||(globalThis.__MEX_FUNDING_CACHE__=new Map),Se=45e3,Ce="/assets/img/payment_methods/",xe=[{name:"Google Pay",file:"pm-google-pay.png"},{name:"Apple Pay",file:"pm-apple-pay.png"},{name:"SII",file:"pm-sii.png"},{name:"American Express",file:"pm-amex.png"},{name:"Mastercard",file:"pm-mastercard.png"},{name:"Visa",file:"pm-visa.png"},{name:"USDT",file:"pm-usdt-networks.png"},{name:"Bank transfer",file:"pm-bank-transfer.png"}];let M=[];const ae=[{key:"deposit",label:"funding.deposit",fallback:"Deposit",icon:p.deposit},{key:"withdraw",label:"funding.withdraw",fallback:"Withdraw",icon:p.withdraw},{key:"history",label:"funding.history",fallback:"History",icon:p.wallet}];function ut(e={}){const t=ge(e),s=x("wallet")||{},r=x("mode")==="real"?"real":"demo",i=!H(),a=t!=="history"&&(r!=="real"||i),o=r==="real"?s.real||{}:s.demo||{};return`
    ...
    </div>`}function ct(e,t={}){const s=ge(t);qe(e,t);const r=o=>{const d=o.target.closest("[data-funding-tab]");if(d){o.preventDefault(),re("wallet",{action:d.dataset.fundingTab||"deposit"});return}if(o.target.closest("[data-switch-real]")){o.preventDefault(),$e("funding");return}if((x("mode")!=="real"||!H())&&s!=="history"&&o.target.closest("[data-funding-category], [data-method], [data-quick-amount], [data-copy-address], #funding-form, .funding-locked-shell")){o.preventDefault(),Y({body:n("gate.funding_body","Deposits and withdrawals are available for verified real accounts only.")});return}const u=o.target.closest("[data-funding-category]");if(u){e.__fundingCategory=u.dataset.fundingCategory||"",e.__fundingSelectedMethodId="",ie(e),oe(e),S(e);return}const h=o.target.closest("[data-method]");if(h){e.__fundingSelectedMethodId=h.dataset.method||"",S(e);return}if(o.target.closest("[data-dismiss-promo]")){o.preventDefault();const g=e.querySelector("[data-promo-banner]");g&&(g.style.display="none");return}const _=o.target.closest("[data-history-filter]");if(_){e.__fundingHistoryFilter=_.dataset.historyFilter||"all",le(e);return}const b=o.target.closest("[data-funding-history-toggle]");if(b){const g=b.closest("[data-funding-history-card]");if(g){const k=!g.classList.contains("is-expanded");g.classList.toggle("is-expanded",k),b.setAttribute("aria-expanded",k?"true":"false")}return}const $=o.target.closest("[data-copy-address]");if($){tt($);return}};e.addEventListener("click",r),M.push(()=>e.removeEventListener("click",r));const i=o=>{o.target.matches('input[name="amount"], [data-withdraw-field]')&&S(e)};e.addEventListener("input",i),M.push(()=>e.removeEventListener("input",i));const a=o=>{const d=o.target.closest('input[name="proof"]');if(!d)return;const u=e.querySelector("#proof-file-name"),h=d.files?.[0];u&&(u.textContent=h?`${h.name} - ${dt(h.size||0)}`:n("funding.upload_file_hint","Image or PDF up to 8MB"))};if(e.addEventListener("change",a),M.push(()=>e.removeEventListener("change",a)),e.querySelectorAll("[data-quick-amount]").forEach(o=>{o.addEventListener("click",()=>{const d=e.querySelector('input[name="amount"]');d&&(d.value=o.dataset.quickAmount||""),S(e)})}),s==="history"){Be(e);return}Le(e,s),e.querySelector("#funding-form")?.addEventListener("submit",o=>{if(x("mode")!=="real"||!H()){o.preventDefault(),Y({body:n("gate.funding_body","Deposits and withdrawals are available for verified real accounts only.")});return}We(o,e,s)})}function Te(e){return`<section class="funding-locked-shell blur-gate blur-active">
    ...
    <div class="deposit-transfer-ready">
      ...
      ${Pe(t)}
    </div>`,o&&Je(e,u)}function J(e,t=!1){const s=e.max_amount?m(e.max_amount):n("funding.flexible","Flexible");return`<div class="funding-route-summary" aria-label="Selected funding route summary">
    ...
    <div class="funds-history-amount"><strong>${t?"+":""}${m(e.amount)} ${l(e.currency)}</strong><span class="${me(e.status)}">${l(W(e.status))}</span></div>
