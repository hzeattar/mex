import{j as _,k as V,g as b,t as n,h as S,m as y,e as D,r as ie,c as x,s as N,S as de,L as J,b as X,p as Y,n as ue,o as G}from"./main-CAGd4vJO.js";import{marketIconPath as pe,marketInitial as me}from"./marketIcon-D-Yq8Sis.js";let T=null,M=[];const re=[{code:"USD",flag:"🇺🇸",name:"US Dollar"},{code:"EGP",flag:"🇪🇬",name:"Egyptian Pound"},{code:"SAR",flag:"🇸🇦",name:"Saudi Riyal"},{code:"AED",flag:"🇦🇪",name:"UAE Dirham"},{code:"KWD",flag:"🇰🇼",name:"Kuwaiti Dinar"},{code:"QAR",flag:"🇶🇦",name:"Qatari Riyal"},{code:"BHD",flag:"🇧🇭",name:"Bahraini Dinar"},{code:"OMR",flag:"🇴🇲",name:"Omani Rial"},{code:"JOD",flag:"🇯🇴",name:"Jordanian Dinar"},{code:"TRY",flag:"🇹🇷",name:"Turkish Lira"},{code:"EUR",flag:"🇪🇺",name:"Euro"},{code:"GBP",flag:"🇬🇧",name:"British Pound"}];function je(){const e=_("brand")||{},a=Q(),r=(_("portfolio")||{}).metrics||{},o=_("mode")==="real"?"real":"demo",s=_("level")||{},l=s.current||{},i=s.next||{},p=Number(s.confirmed_deposit_total||s.total_deposits||s.deposit_total||0),d=Number(i.min_deposit_total||i.min_total_deposit||i.required_deposit||0),m=Math.max(0,d-p),c=d>0?Math.min(100,Math.round(p/d*100)):p>0?100:0,h=Number(a.balance||a.available||0),v=Number(r.total_balance??h),u=Number(r.available_balance??a.available??v??0),f=Number(r.in_use_balance??a.holds??a.locked??0),$=Number(r.pnl_24_live??0),g=Number(r.pnl_total_live??0),k=l.name||l.name_en||l.level_code||"Starter";return i.name||i.name_en,`
    <div class="pro-dashboard animate-fade-in pro-dashboard-compact">
      ...
      <section class="pro-card pro-level-scroll-card">
        ...
        <div class="pro-level-rail" aria-label="Level benefits and next tiers">
          ${oe(s,{currentLevel:l,nextLevel:i,remainingToNext:m,nextRequired:d,levelProgress:c,mode:o})}
        </div>
      </section>
        ...
        <div class="pro-main-stack">
          <section class="pro-card pro-card-section">
            ...
            <div class="pro-market-tabs" id="home-market-tabs">
              ${[["all",n("common.all","All")],["crypto",n("markets.crypto","Crypto")],["forex",n("markets.forex","Forex")],["stocks",n("markets.stocks","Stocks")],["commodities",n("markets.commodities","Commodities")],["futures",n("markets.futures","Futures")],["arab",n("markets.arab","Arab")]].map(([C,q],w)=>`<button class="${w===0?"active":""}" data-home-tab="${C}">${b(q)}</button>`).join("")}
            </div>
  ...
  </div>`}function F(e){const a=e.querySelector("[data-home-lang-dropdown]"),t=e.querySelector("[data-home-lang-overlay]"),r=e.querySelector("[data-home-lang-trigger]");a&&a.classList.add("hidden"),t&&t.classList.add("hidden"),r&&r.setAttribute("aria-expanded","false")}function Te(e,a,t,r=""){const o=e.querySelector("#home-pnl-chart-value"),s=e.querySelector("#home-pnl-spark");o&&(o.textContent=`${y(a)}${r?" "+r:""}`,o.className=a>=0?"text-buy":"text-sell"),s&&(s.innerHTML=ne(a,t))}function ne(e=0,a=0){const t=Number(e||0),r=Number(a||t||1),o=Array.from({length:12},(m,c)=>{const h=Math.sin((c+1)*.9)*Math.max(12,Math.abs(r)*.025);return t*(c/11)+h}),s=Math.min(...o,0),l=Math.max(...o,1),i=Math.max(1,l-s),p=o.map((m,c)=>{const h=5+c*10,v=54-(m-s)/i*44;return`${h.toFixed(1)},${v.toFixed(1)}`}).join(" "),d=t>=0?"var(--buy)":"var(--sell)";return`<svg viewBox="0 0 120 64" role="img" aria-label="PnL total chart" preserveAspectRatio="none">
