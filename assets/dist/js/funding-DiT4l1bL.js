import{j as v,k as c,h as f,t as n,m as p,g as d,n as me,s as he,l as _e,i as ye,c as be}from"./main-4sNiSzdn.js";const N=globalThis.__MEX_FUNDING_CACHE__||(globalThis.__MEX_FUNDING_CACHE__=new Map),$e=45e3;let T=[];const Z=[{key:"deposit",label:"funding.deposit",fallback:"Deposit",icon:c.deposit},{key:"withdraw",label:"funding.withdraw",fallback:"Withdraw",icon:c.withdraw},{key:"history",label:"funding.history",fallback:"History",icon:c.wallet}];function Ze(e={}){const t=de(e),i=v("wallet")||{};v("kyc");const r=v("level")||{},a=v("mode")==="real"?"real":"demo",s=a==="real"?i.real||{}:i.demo||{};return r.current,`
    <div class="funds-workspace animate-fade-in" data-active-funding-tab="${f(t)}">
      ...
      ${t==="history"?we():ve(t)}
    </div>`}function et(e,t={}){const i=de(t),r=o=>{const l=o.target.closest("[data-funding-tab]");if(l){o.preventDefault(),me("wallet",{action:l.dataset.fundingTab||"deposit"});return}const u=o.target.closest("[data-funding-category]");if(u){e.__fundingCategory=u.dataset.fundingCategory||"",e.__fundingSelectedMethodId="",ee(e),te(e),S(e);return}const g=o.target.closest("[data-method]");if(g){e.__fundingSelectedMethodId=g.dataset.method||"",S(e);return}const C=o.target.closest("[data-history-filter]");if(C){e.__fundingHistoryFilter=C.dataset.historyFilter||"all",re(e);return}const m=o.target.closest("[data-funding-history-toggle]");if(m){const y=m.closest("[data-funding-history-card]");if(y){const _=!y.classList.contains("is-expanded");y.classList.toggle("is-expanded",_),m.setAttribute("aria-expanded",_?"true":"false")}return}const $=o.target.closest("[data-copy-address]");if($){ze($);return}o.target.closest("[data-switch-real]")&&(localStorage.setItem("vp_mode","real"),he("mode","real"),location.reload())};e.addEventListener("click",r),T.push(()=>e.removeEventListener("click",r));const a=o=>{o.target.matches('input[name="amount"], [data-withdraw-field]')&&S(e)};e.addEventListener("input",a),T.push(()=>e.removeEventListener("input",a));const s=o=>{const l=o.target.closest('input[name="proof"]');if(!l)return;const u=e.querySelector("#proof-file-name"),g=l.files?.[0];u&&(u.textContent=g?`${g.name} - ${Ye(g.size||0)}`:n("funding.upload_file_hint","Image or PDF up to 8MB"))};if(e.addEventListener("change",s),T.push(()=>e.removeEventListener("change",s)),e.querySelectorAll("[data-quick-amount]").forEach(o=>{o.addEventListener("click",()=>{const l=e.querySelector('input[name="amount"]');l&&(l.value=o.dataset.quickAmount||""),S(e)})}),i==="history"){Ie(e);return}Se(e,i),e.querySelector("#funding-form")?.addEventListener("submit",o=>Fe(o,e,i))}function tt(){document.querySelectorAll(".funds-workspace").forEach(e=>F(e)),T.forEach(e=>{try{e()}catch{}}),T=[]}function ve(e){const t=e==="deposit";return`
      ...
      <aside class="funding-side-stack">
        <section class="card funding-sidebar-card funding-guide-card">
          ...
          <div class="funding-checklist">
            ...
            ${D(n("funding.check_real_required","Real mode required"),v("mode")==="real")}
          </div>
        </section>
      </aside>
    ...
    </section>`}async function H(e,t={},i=$e){const r=e,a=Date.now(),s=N.get(r);if(s&&a-s.time<i)return s.data;const o=await be(e,t);return N.set(r,{time:a,data:o}),o}async function Se(e,t){try{const i=v("mode")==="real"?"real":"both",r=await H(`/payment_methods/list.php?kind=${encodeURIComponent(t)}&scope=${i}&currency=*`,{timeout:9e3,retry:1},9e4),a=r?.items||[],s=ke(r?.categories||[],a);e.__fundingKind=t,e.__fundingMethods=a,e.__fundingCategories=s,e.__fundingCategory=s[0]?.key||"",e.__fundingSelectedMethodId="",ee(e),te(e),S(e)}catch{const r=e.querySelector("#funding-categories"),a=e.querySelector("#method-cards");r&&(r.innerHTML=`<div class="empty-state empty-state--compact">${n("funding.sections_unavailable","Payment sections are temporarily unavailable.")}</div>`),a&&(a.innerHTML="")}}function ke(e,t){const i=new Set(t.map(k).filter(Boolean)),r=[];return e.forEach(a=>{const s=M(a.key||a.key_slug||a.label);!s||!i.has(s)||r.push({key:s,label:a.label||L(s),hint:a.hint||J(s),icon:a.image_url?`<img src="${f(a.image_url)}" alt="">`:j(s,a.icon)})}),i.forEach(a=>{r.some(s=>s.key===a)||r.push({key:a,label:L(a),hint:J(a),icon:j(a)})}),r}function ee(e){const t=e.querySelector("#funding-categories"),i=e.__fundingCategories||[],r=e.__fundingCategory||i[0]?.key||"";if(t){if(!i.length){t.innerHTML=`<div class="empty-state empty-state--compact">${n("funding.no_sections","No active funding sections are configured by admin.")}</div>`;return}t.innerHTML=i.map(a=>`
    <button type="button" class="${a.key===r?"active":""}" data-funding-category="${f(a.key)}">
      <i>${a.icon||c.wallet}</i>
      <strong>${d(a.label)}</strong>
      <small>${d(a.hint||"")}</small>
    </button>
  `).join("")}}function te(e){const t=e.querySelector("#method-cards"),i=ae(e);if(t){if(F(e),!i.length){t.innerHTML=`<div class="empty-state empty-state--compact">${n("funding.no_methods_section","No active methods under this section.")}</div>`,e.__fundingSelectedMethodId="",S(e);return}i.some(r=>w(r)===e.__fundingSelectedMethodId)||(e.__fundingSelectedMethodId=w(i[0])),t.innerHTML=i.map(r=>{const a=w(r);return`<button type="button" class="method-card ${a===e.__fundingSelectedMethodId?"active":""}" data-method="${f(a)}">
  ...
  </div>`}function x(e,t){return`<span class="funding-route-pill"><small>${d(e)}</small><b>${d(t)}</b></span>`}function G(e,t,i){const r=e.querySelector("#deposit-proof-slot");if(!r)return;if(!t){r.dataset.methodId="",r.dataset.isStripe="",r.innerHTML="";return}const a=w(t),s=i?"1":"0";if(!(r.dataset.methodId===a&&r.dataset.isStripe===s&&r.innerHTML.trim()!=="")){if(r.dataset.methodId=a,r.dataset.isStripe=s,i){r.innerHTML=`<div class="deposit-proof-note">
      <span>${c.wallet}</span>
  ...
  </label>`}}function X(e,t){const i=e.querySelector("#withdraw-fields-panel");if(!i)return;if(!t){i.innerHTML=`<p class="text-muted text-sm">${n("funding.select_withdraw_method_first","Select a withdrawal method first.")}</p>`;return}const r=Le(t.fields||[]);if(!r.length){i.innerHTML=`<label class="block">
