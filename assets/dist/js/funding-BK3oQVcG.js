import{g as p,a as r,e as i,m as u,h as m}from"./main-BXbPUX4a.js";function g(s){const a=(s.path||p("route")||"deposit").includes("withdraw")?"withdraw":"deposit",t=a==="deposit";return`
    ...
    </div>`}function y(s,a){const t=s.querySelector("#funding-form")?.dataset.kind||"deposit";f(s,t),c(s,t),s.querySelector("#funding-form")?.addEventListener("submit",o=>x(o,s,t))}async function f(s,a){try{const t=await r("/payment_methods/list.php?kind="+a,{timeout:6e3}),o=s.querySelector("#method-select");if(!o||!t||!t.items)return;o.innerHTML=(t.items||[]).map(e=>`<option value="${i(e.id||e.code||"")}">${i(e.name||e.label||"Method")}</option>`).join("")||'<option value="">No methods available</option>'}catch{}}async function c(s,a){try{const o=await r(a==="deposit"?"/deposits/list.php":"/withdrawals/list.php",{timeout:6e3}),e=s.querySelector("#funding-history");if(!e||!o)return;const d=o.items||[];if(!d.length){e.innerHTML=`<p class="text-muted text-sm text-center py-4">No ${a} history.</p>`;return}e.innerHTML=`<div class="space-y-2">${d.slice(0,10).map(n=>`
      <div class="flex items-center justify-between p-3 rounded-lg bg-panel-2/50 border border-line/50">
        <div>
          <div class="text-sm font-semibold">${u(n.amount)} ${i(n.currency||"USDT")}</div>
          <div class="text-[11px] text-muted">${i(n.method_label||n.method||"--")} - ${i(n.created_at||"")}</div>
        </div>
        <span class="badge ${b(n.status)}">${i(n.status||"pending")}</span>
      </div>`).join("")}</div>`}catch{}}async function x(s,a,t){s.preventDefault();const o=s.target,e=a.querySelector("#form-status"),d=new FormData(o);d.append("kind",t);try{e&&(e.textContent="Submitting...");const l=await m(t==="deposit"?"/deposits/create.php":"/withdrawals/create.php",d,{timeout:12e3});l&&l.ok!==!1?(e&&(e.textContent="Request submitted!",e.className="text-xs text-center text-green"),c(a,t)):e&&(e.textContent=l?.error||"Failed",e.className="text-xs text-center text-red")}catch(n){e&&(e.textContent=n.message,e.className="text-xs text-center text-red")}}function b(s){const a=(s||"").toLowerCase();return["approved","confirmed","completed","paid"].includes(a)?"badge-green":["pending","requested","processing","review"].includes(a)?"badge-accent":"badge-red"}export{y as mount,g as render};
