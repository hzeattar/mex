import{g as x,e as c,m as v,a as f,i as S,b as $,p as _,n as N,c as b,t as q}from"./main-CoCUWP4H.js";import{m as E,a as A}from"./marketIcon-BqfrwX_4.js";function G(){const e=x("brand")||{},s=z(),a=x("mode");return`
    <div class="space-y-6 animate-fade-in">
    ...
    </div>`}function J(e){U(e)}async function U(e){try{const[s,a,t]=await Promise.all([f("/markets.php?scope=home&supported=1&lite=1&with_quotes=1",{timeout:7e3}),f("/trade/portfolio.php",{timeout:7e3}),f("/signals.php?bot=1&home=1&lang=en",{timeout:7e3}).catch(()=>null)]);if(s&&s.items){const n=s.items.slice(0,12);j(e,n);const i=n.filter(o=>Number(o.price||o.q_price||0)<=0);i.length&&P(e,i)}a&&a.positions&&D(e,a.positions.slice(0,5)),t&&t.items&&t.items.length&&T(e,t.items)}catch{}}function T(e,s){const a=e.querySelector("#home-copy-section");a&&(a.innerHTML=`<div class="flex gap-3 overflow-x-auto pb-2 snap-x">${s.slice(0,6).map(t=>{const n=(t.direction||"BUY").toUpperCase(),i=t.symbol||t.market_symbol||"--";return`<div class="shrink-0 w-[280px] copy-card snap-start">
      ...
      ${V(t)?'<p class="text-[10px] text-muted mt-2">Awaiting desk levels</p>':""}
  ...
  `).join(""),a.querySelectorAll("[data-symbol]").forEach(t=>{t.addEventListener("click",()=>N("trade",{symbol:t.dataset.symbol,type:t.dataset.type}))}))}async function P(e,s){const a=new Map;s.forEach(n=>{const i=String(n.symbol||"").toUpperCase(),o=n.type||"crypto";i&&(a.has(o)||a.set(o,[]),a.get(o).push(i))});const t=[];await Promise.all([...a.entries()].map(async([n,i])=>{const o=await f(`/quotes.php?symbols=${encodeURIComponent(i.join(","))}&type=${encodeURIComponent(n)}&visible=1&purpose=watchlist`,{timeout:6500}).catch(()=>null),l=new Set;o?.items?.length&&o.items.forEach(r=>{l.add(String(r.symbol||"").toUpperCase()),C(e,r,n)}),i.forEach(r=>{const d=L(e,r),p=d&&d.querySelector("[data-home-price]")?.textContent!=="--";(!l.has(r)||!p)&&t.push({symbol:r,type:n})})})),R(e,t)}async function R(e,s){const a=new Map;s.forEach(t=>{!t.symbol||!t.type||(a.has(t.type)||a.set(t.type,[]),a.get(t.type).push(t.symbol))});for(const[t,n]of a.entries()){const i=t==="crypto"?12:2,o=t==="crypto"?12:4,l=[...new Set(n)].slice(0,o);for(let r=0;r<l.length;r+=i){const d=l.slice(r,r+i),p=await f(`/quotes.php?symbols=${encodeURIComponent(d.join(","))}&type=${encodeURIComponent(t)}&cache_only=1&purpose=watchlist`,{timeout:3500}).catch(()=>null);p?.items?.length&&p.items.forEach(y=>C(e,y,t))}}}function C(e,s,a){const t=String(s.symbol||"").toUpperCase(),n=L(e,t);if(!n)return;const i=n.dataset.type||s.type||a,o=Number(s.price||s.q_price||0),l=Number(s.change_pct||s.q_change||0),r=n.querySelector("[data-home-price]"),d=n.querySelector("[data-home-change]"),p=n.querySelector("[data-home-source]");r&&o>0&&(r.textContent=b(o,i)),d&&(d.textContent=_(l),d.className=`text-[11px] ${l>=0?"text-green":"text-red"}`),p&&o>0&&(p.textContent=h(s));const y=n.querySelector("[data-quote-chip]");y&&o>0&&(y.className=`status-chip ${M(s)}`,y.textContent=h(s))}function L(e,s){return[...e.querySelectorAll("[data-symbol]")].find(a=>String(a.dataset.symbol||"").toUpperCase()===String(s||"").toUpperCase())}function w(e,s){const a=e.symbol||"--";return`<span class="${s}">
