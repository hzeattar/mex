import{f as v,s as y,j as l,t as e,m as d,g as s,c as f,e as k,l as N,k as I,p as Y,h as E}from"./main-D92-0-Cl.js";let m=[];function tt(){const t=l("invest.tab")||localStorage.getItem("vp_earn_tab")||"copy",a=l("level")||{},n=Q();a.current,a.next;const o=l("mode");return`
    ...
    </div>`}function et(t){C(t),setTimeout(()=>{const a=t.querySelector(".level-strip"),n=a?.querySelector(".level-pill.is-current");a&&n&&n.scrollIntoView({inline:"center",behavior:"smooth",block:"nearest"})},100),m.push(v(t,"[data-earn-tab]","click",(a,n)=>{y("invest.tab",n.dataset.earnTab),localStorage.setItem("vp_earn_tab",n.dataset.earnTab),w(t)})),m.push(v(t,"[data-copy-signal]","click",(a,n)=>H(n.dataset.copySignal,t))),m.push(v(t,"[data-cancel-copy]","click",(a,n)=>F(n.dataset.cancelCopy,t))),m.push(v(t,"[data-contract-subscribe]","click",(a,n)=>G(n.dataset.contractSubscribe,t))),m.push(v(t,"[data-switch-real]","click",()=>{localStorage.setItem("vp_mode","real"),y("mode","real"),location.reload()})),m.push(v(t,"[data-open-kyc]","click",()=>{location.hash="#/kyc"}))}function at(){m.forEach(t=>{try{t()}catch{}}),m=[]}async function C(t){try{const[a,n,o,i]=await Promise.all([f(`/signals.php?bot=1&home=1&lang=${encodeURIComponent(k())}`,{timeout:8e3}).catch(()=>({items:[]})),f(`/invest/contracts.php?lang=${encodeURIComponent(k())}`,{timeout:8e3}).catch(()=>({items:[]})),f(`/trading_bot/my.php?lang=${encodeURIComponent(k())}`,{timeout:8e3}).catch(()=>({items:[]})),f(`/invest/my.php?lang=${encodeURIComponent(k())}`,{timeout:8e3}).catch(()=>({items:[]}))]);y("invest.signals",a.items||[]),y("invest.contracts",n.items||[]),y("invest.copies",o.items||[]),y("invest.mine",{contracts:(i.items||[]).filter(c=>String(c.product_kind||"").toLowerCase()==="contract")}),w(t)}catch{w(t)}}function w(t){const a=t.querySelector("#invest-content");if(!a)return;const n=l("invest.tab")||localStorage.getItem("vp_earn_tab")||"copy",o=T(),i=n==="copy"?j():M();a.innerHTML=o?z(i,o):i,t.querySelectorAll("[data-earn-tab]").forEach(c=>{c.classList.toggle("active",c.dataset.earnTab===n)})}function j(){const t=l("invest.signals")||[],a=l("invest.copies")||[],n=a.filter(A),o=a.filter(i=>!A(i));return`
    ...
    </section>`}function O(t){const a=t.symbol||t.market_symbol||"--",n=String(t.direction||"BUY").toUpperCase(),o=S(n),i=t.type||t.market_type||"crypto",c=Number(t.live_price||0),p=c>0?e("market.live","LIVE"):e("market.unavailable","UNAVAILABLE"),u=c>0?"status-chip-live":"status-chip-locked",_=c>0?`$${d(c,i==="forex"?5:2)}`:"--",g=c>0?Y(t.live_change_pct||0):"0.00%",B=Number(t.copy_min_amount||100),P=t.levels_source==="live_derived"?'<span class="status-chip status-chip-derived">Live derived levels</span>':"";return`<article class="copy-card">
  ...
  </article>`}function V(t){const a=t.eligible!==!1,n=Number(t.is_perpetual||0)===1?e("earn.perpetual","Perpetual"):`${Number(t.term_days||0)}${e("common.days_short","d")}`,o=Number(t.min_amount||0),i=a?e("earn.eligible","ELIGIBLE"):e("earn.level_locked","LEVEL LOCKED");return`<article class="contract-card ${a?"":"locked"}">
    <div class="copy-card__top">
      <div>
  ...
  </article>`}function L(t){const a=String(t||"active").toLowerCase();return{active:e("bot.status_active","Active"),armed:e("bot.status_armed","Armed"),copied:e("bot.status_copied","Copied"),open:e("bot.status_open","Open"),running:e("bot.status_running","Running"),pending:e("bot.status_pending","Pending"),closed:e("bot.status_closed","Closed"),canceled:e("bot.status_canceled","Canceled"),cancelled:e("bot.status_canceled","Canceled"),expired:e("bot.status_expired","Expired")}[a]||String(t||"").toUpperCase()}function U(t){const a=t.symbol||t.market_symbol||"--",n=L(t.status),o=A(t),i=Array.isArray(t.open_positions)?t.open_positions:[],c=Array.isArray(t.closed_positions)?t.closed_positions:[],p=Number(t.pnl_total||0);return`<article class="history-card">
  ...
  </article>`}function H(t,a){const n=(l("invest.signals")||[]).find(c=>String(c.id)===String(t));if(!n)return;const o=T();if(o){D(o);return}const i=Number(n.copy_min_amount||100);x(`
  ...
  `),document.querySelector("#copy-form")?.addEventListener("submit",async c=>{c.preventDefault();const p=c.currentTarget,u=p.querySelector("#copy-error"),_=Number(new FormData(p).get("amount")||0);try{await N("/trading_bot/copy.php",{signal_id:Number(t),amount:_,mode:"real"},{timeout:12e3}),b(),await C(a)}catch(g){u&&(u.textContent=g?.message||e("bot.copy_failed","Copy failed"),u.classList.remove("hidden"))}})}async function F(t,a){if(!(!t||!window.confirm(e("bot.cancel_confirm","Cancel this Avalon copy and close its open trades at the current market price?"))))try{await N("/trading_bot/cancel.php",{subscription_id:Number(t)},{timeout:15e3}),await C(a)}catch(o){x(`<div class="text-center space-y-3">
    ...
    </div>`)}}function G(t,a){const n=(l("invest.contracts")||[]).find(c=>String(c.id)===String(t));if(!n)return;const o=T();if(o){D(o);return}const i=Number(n.min_amount||0);x(`
