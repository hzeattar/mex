import{j as _,l as ie,k as S,g as v,v as n,h as k,m as y,e as P,t as ge,c as x,s as w,S as ye,L as re,b as oe,x as _e,q as X,p as Z,n as $e}from"./main-B1KKnkFD.js";import{marketIconPath as ke,marketInitial as xe}from"./marketIcon-D-Yq8Sis.js";let E=null,T=[];const de=[{code:"USD",flag:"🇺🇸",name:"US Dollar"},{code:"EGP",flag:"🇪🇬",name:"Egyptian Pound"},{code:"SAR",flag:"🇸🇦",name:"Saudi Riyal"},{code:"AED",flag:"🇦🇪",name:"UAE Dirham"},{code:"KWD",flag:"🇰🇼",name:"Kuwaiti Dinar"},{code:"QAR",flag:"🇶🇦",name:"Qatari Riyal"},{code:"BHD",flag:"🇧🇭",name:"Bahraini Dinar"},{code:"OMR",flag:"🇴🇲",name:"Omani Rial"},{code:"JOD",flag:"🇯🇴",name:"Jordanian Dinar"},{code:"TRY",flag:"🇹🇷",name:"Turkish Lira"},{code:"EUR",flag:"🇪🇺",name:"Euro"},{code:"GBP",flag:"🇬🇧",name:"British Pound"}];function st(){const e=_("brand")||{},t=ae(),r=(_("portfolio")||{}).metrics||{},s=_("mode")==="real"?"real":"demo",o=Y(),l=_("level")||{},c=pe(l),i=c.currentLevel,p=c.nextLevel,m=c.confirmedTotal,d=c.nextRequired,h=c.remainingToNext,b=c.levelProgress,u=s==="demo"&&!ie(),f=Number(t.balance||t.available||0),g=Number(r.total_balance??f),$=Number(r.available_balance??t.available??g??0),C=Number(r.in_use_balance??t.holds??t.locked??0),N=Number(r.pnl_24_live??0),L=Number(r.pnl_total_live??0),q=O(i,0);return`
  ...
  </div>`}function K(){return U(localStorage.getItem("vp_home_currency")||"USD")}function U(e){const t=String(e||"USD").toUpperCase().replace(/[^A-Z]/g,"").slice(0,3);return de.some(a=>a.code===t)?t:"USD"}function M(e){return["JPY"].includes(e)?0:2}function je(){const e=K(),t=_("home.fx")||{};return U(t.code)===e&&Number(t.rate||0)>0?{code:e,rate:Number(t.rate),source:t.source||"",error:!!t.error}:{code:e,rate:1,source:e==="USD"?"base":"",error:!1}}async function le(e,t=K()){const a=U(t);if(localStorage.setItem("vp_home_currency",a),a==="USD"){w("home.fx",{code:"USD",rate:1,source:"base",updated_at:Math.floor(Date.now()/1e3)}),V(e);return}const r=_("home.fx")||{};U(r.code)===a&&Number(r.rate||0)>0&&V(e);try{const s=await x(`/fx_rate.php?to=${encodeURIComponent(a)}`,{timeout:4e3,retry:1,cacheTtl:36e5}),o=Number(s?.rate||0);o>0?w("home.fx",{code:a,rate:o,source:s.source||"fx",updated_at:Number(s.updated_at||0),error:!1}):w("home.fx",{code:a,rate:Number(r.rate||1),source:s?.source||"fx",updated_at:Number(s?.updated_at||0),error:!0})}catch{w("home.fx",{code:a,rate:Number(r.rate||1),source:r.source||"cached",updated_at:Number(r.updated_at||0),error:!0})}V(e)}function Fe(){const e=P(),t=ye.map(a=>`<button type="button" class="home-lang-option${a===e?" is-active":""}" data-home-set-locale="${k(a)}" data-lang="${k(a)}">
  ...
  </svg>`}function Ve(e){return e!=="real"?`<div class="relative min-h-[180px]">
  ...
  </div>`}function ae(){const e=_("wallet")||{};return _("mode")==="real"?e.real||{balance:0,available:0,currency:"USDT"}:e.demo||{balance:1e4,available:1e4,currency:"USDT_DEMO"}}function Ge(){return`<section class="home-kyc-banner">
