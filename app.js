...
const QuoteCache = (() => {
  ...
  function preferredQuotePollMs(type='crypto', market='spot', hidden=false){
    ...
    if(hidden){
      ...
      if(norm === 'commodities') return 4000;
      if(norm === 'futures') return 5000;
      if(norm === 'forex') return 3500;
      if(norm === 'stocks' || norm === 'arab') return 18000;
      return 14000;
    }
    if(norm === 'crypto') return 400;
    if(norm === 'commodities') return 1000;
    if(norm === 'futures') return 1500;
    if(norm === 'forex') return 800;
    if(norm === 'stocks' || norm === 'arab') return 6000;
    return 5000;
  }
  ...
  function setSpeed(ms){
    ms = Math.max(200, Math.min(30000, Number(ms)||400));
...
async function refreshMarkets(opts={}){
  ...
  const ttlMs = Number(opts.ttlMs || (selectedType==='crypto' ? 250 : 900));
...
function marketsPage(){
  ...
  let sseConnection = null; // SSE live connection for price streaming
  let sseHydrateWave = 0;   // track SSE lifecycle per hydration wave
  ...
  if(state.selectedAssetType !== 'crypto' && !state.__nonCryptoMarketsWarmStarted){
    state.__nonCryptoMarketsWarmStarted = true;
    Promise.resolve().then(async ()=>{
      try{
        await Promise.allSettled([
          refreshMarkets({type:'forex', lite:true, withQuotes:true, applyToState:false, warm:true, ttlMs:700}),
          refreshMarkets({type:'stocks', lite:true, withQuotes:true, applyToState:false, warm:true, ttlMs:700}),
          refreshMarkets({type:'arab', lite:true, withQuotes:true, applyToState:false, warm:true, ttlMs:700}),
          refreshMarkets({type:'commodities', lite:true, withQuotes:true, applyToState:false, warm:true, ttlMs:700})
        ]);
        if(state.selectedAssetType !== 'crypto') requestRender();
      }catch(e){}
    });
  }
  ...
  // SSE-based live price streaming: connects directly to /api/stream/sse.php
  // and pushes price updates into the market list without full page polling.
  // Inline EventSource — no external import needed for the monolithic build.
  function startSSEHydration(){
    sseHydrateWave++;
    const wave = sseHydrateWave;
    // Close previous connection
    try{ if(sseConnection && sseConnection.close) sseConnection.close(); }catch(e){}
    try{ if(sseConnection && typeof sseConnection === 'function') sseConnection(); }catch(e){}
    sseConnection = null;

    const typeNorm = normalizeLiveAssetType(state.selectedAssetType || 'crypto');
    const syms = (items || []).slice(0, 36).map(it => String(it?.symbol || '').toUpperCase()).filter(Boolean);
    if(!syms.length || typeof EventSource !== 'function') return;

    const url = '/api/stream/sse.php?symbols=' + encodeURIComponent(syms.join(',')) + '&type=' + encodeURIComponent(typeNorm) + '&scope=watchlist&_=' + Date.now();
    let es = null;
    let aliveWatchdog = null;
    const killSSE = () => {
      try{ if(aliveWatchdog) clearTimeout(aliveWatchdog); }catch(e){}
      try{ if(es) es.close(); }catch(e){}
      es = null;
      if(sseConnection === killSSE) sseConnection = null;
    };

    const armWatchdog = () => {
      try{ if(aliveWatchdog) clearTimeout(aliveWatchdog); }catch(e){}
      aliveWatchdog = setTimeout(() => { killSSE(); }, 12000);
    };

    try {
      es = new EventSource(url, { withCredentials: true });
      sseConnection = killSSE;

      es.addEventListener('open', () => { armWatchdog(); });

      // Snapshot: initial full data
      es.addEventListener('snapshot', (event) => {
        if(!alive || wave !== sseHydrateWave) return killSSE();
        try {
          const items = JSON.parse(event.data || '[]');
          if(Array.isArray(items) && items.length) mergeQuoteItems(items);
          armWatchdog();
        } catch(e){}
      });
      ...
      es.addEventListener('reconnect', () => { killSSE(); });
      es.addEventListener('error', () => { killSSE(); });

      // Fallback: if no message in 8s, close SSE (polling takes over)
      armWatchdog();
    } catch(e) {
      killSSE();
    }
  ...
  function startBatchHydration(){
    ...
    // Start SSE live streaming for faster price updates
    startSSEHydration();
    ...
    if(typeNorm !== 'crypto'){
      ...
      if(onTradeRoute){
        pollTimer = setInterval(async ()=>{
          ...
          const chunks = quoteChunks();
          const firstChunk = chunks[0] || [];
          if(firstChunk.length) await fetchQuoteChunk(firstChunk, typeNorm === 'forex' ? 600 : (typeNorm === 'commodities' ? 600 : 800));
        }, typeNorm === 'forex' ? 1500 : (typeNorm === 'commodities' ? 1400 : (typeNorm === 'futures' ? 1600 : 2000)));
        return;
      }
      const runCascade = async(idx=0)=>{
        ...
        await fetchQuoteChunk(chunks[idx], idx === 0 ? 500 : 700);
        if(!alive || wave !== hydrateWave) return;
        cascadeTimer = setTimeout(()=>{ runCascade(idx + 1); }, idx === 0 ? 150 : 250);
      };
      runCascade(1);
      pollTimer = setInterval(async ()=>{
        if(!alive || wave !== hydrateWave || isEffectivelyHidden()) return;
        ...
        await fetchQuoteChunk(chunks[idx] || [], typeNorm === 'commodities' ? 600 : (typeNorm === 'forex' ? 600 : 900));
      }, typeNorm === 'forex' ? 2000 : (typeNorm === 'commodities' ? 1800 : (typeNorm === 'futures' ? 2000 : 2400)));
      return;
    }
    ...
    const runCascade = async(idx=0)=>{
      ...
      const chunks = quoteChunks();
      if(!chunks.length || idx >= chunks.length) return;
      const ttlMs = (idx === 0) ? 50 : 150;
      await fetchQuoteChunk(chunks[idx], ttlMs);
      if(!alive || wave !== hydrateWave) return;
      cascadeTimer = setTimeout(()=>{ runCascade(idx + 1); }, idx === 0 ? 30 : 60);
    };

    runCascade(0);

    // Faster crypto polling: 1200ms instead of 1800ms (SSE handles most updates)
    pollTimer = setInterval(async ()=>{
      if(!alive || wave !== hydrateWave || isEffectivelyHidden()) return;
      const chunks = quoteChunks();
      if(!chunks.length) return;
      ...
      const ttlMs = hasVisible ? 180 : 400;
...
async function primePlatformMarketUniverse(opts={}){
  ...
  const run = (async()=>{
    ...
    for(let i=0;i<order.length;i+=2){
      ...
      if(i + 2 < order.length) await new Promise(resolve=>setTimeout(resolve, i === 0 ? 60 : 90));
    }
    try{
