...
$fastQuotes = ((int)($_GET['fast'] ?? 0) === 1) || $homeOnly;
...
try {
  ...
  if ($symbolsByType) {
    ...
    foreach ($symbolsByType as $assetType => $symbolMap) {
      ...
      $liveRows = [];
      if ($fastQuotes) {
        // Home dashboard must paint instantly. Use warmer/cache rows only here;
        // full live refresh belongs to quotes.php/quote_active.php and cron warmers.
        foreach ($symbols as $sym) {
          try {
            $cached = quote_get($sym, $assetType);
            if (is_array($cached) && (float)($cached['price'] ?? 0) > 0) $liveRows[$sym] = $cached;
          } catch (Throwable $e) {}
        }
      } else {
        try {
          $liveRows = quote_bulk_live($symbols, $assetType, $metaBySymbol, [
            ...
            'yahoo_ttl' => 2,
            'massive_ttl' => 2,
            'direct_budget' => in_array($assetType, ['stocks','arab','futures'], true) ? min(8, count($symbols)) : min(6, count($symbols)),
            'direct_yahoo_budget' => in_array($assetType, ['stocks','arab','futures'], true) ? min(8, count($symbols)) : min(6, count($symbols)),
            'chart_budget' => 0,
          ]);
        } catch (Throwable $e) {
          $liveRows = [];
  ...
  $deriveLevels = static function(float $livePrice, string $direction, string $assetType): array {
    ...
    if ($dir === 'NEUTRAL') return ['entry' => null, 'sl' => null, 'tp1' => null, 'tp2' => null];
    $assetType = strtolower(trim($assetType));
  ...
  foreach ($rows as $r) {
    ...
    if ($botName === '') {
      $assetName = preg_replace('/(USDT|USD|_F)$/', '', $sigSymbol);
      $botName = 'Avalon ' . ($assetName ?: ($sigSymbol ?: 'AI')) . ' AI Bot';
    }
    ...
    if (!(($entry ?? 0) > 0) || !(($sl ?? 0) > 0) || !(($tp1 ?? 0) > 0)) {
      $derived = $deriveLevels($sigLive, $direction, $sigType);
