...
function defaultCacheTtl(path) {
  const p = String(path || '');
  if (p.includes('/markets.php')) return 25000;
  if (p.includes('/wallet/summary.php')) return 12000;
  if (p.includes('/trade/portfolio.php')) return 10000;
  if (p.includes('/trade/orders.php')) return 8000;
  if (p.includes('/invest/contracts.php') || p.includes('/signals.php')) return 20000;
  if (p.includes('/user/level.php')) return 60000;
  if (p.includes('/quotes.php')) {
    if (p.includes('cache_only=1')) return 5000;
    if (p.includes('purpose=watchlist')) return 3000;
  }
  return 0;
}
