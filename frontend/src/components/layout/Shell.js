...
async function loadMarketsList() {
  ...
  try {
    if (!_marketsCache) {
      const data = await api('/markets.php?type=crypto&scope=trade&supported=1&lite=1&with_quotes=1&rescue=1&rescue_noncrypto=1&limit=36', { timeout: 0, retry: 1, cacheTtl: 15000 });
